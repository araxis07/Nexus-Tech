...
- Recorded the 2026-08-20 owner rehearsal as incomplete after a validated isolated run closed with no archive or complete route; human evidence remains unchanged
...
- Extended `prepare-animation-playtest-session` to generate and validate the sprint packet alongside the execution guide, issue backlog, progress board, release gate, UI triage, route batches, recorder queue, plan, command queue, report, and handoff
- Updated docs and regression coverage so stale sprint packets fail validation while real visible-window tester observations remain required before signoff
