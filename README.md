...
validator, and handoff sheet include validated route evidence prompts for all 18
required menu/play window and motion runs. `animation-playtest-next` and
`animation-playtest-handoff` surface the next visible command plus matching
recorder command, so manual QA cannot lose what each visible run must record
between command generation and final signoff.
