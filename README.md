...
The validator checks the embedded deterministic manifest against the current build and evidence store, requires the generated Markdown to remain unchanged, and confirms that every required gameplay profile is still unused and isolated. Regenerate instead of editing a packet when validation fails. This consistency check does not create human evidence or replace reviewer approval.
