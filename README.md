...
- A deterministic decision-quality audit compares that pattern across six campaigns and three difficulties, attributes repeated autoplay actions to explicit policy reasons, and reports default-fallback share; coverage failures block CI, while policy watches, possible gameplay candidates, and low-variety results remain advisory until real-player notes agree
...
- Runtime typography now scales with available standard/spacious viewport room, rebuilds after live resizing, and retains the established compact 820x620 type budget; Focus decision cards use stronger large-screen headline/detail roles while blocking overlays fully separate modal copy from the live scene
...
- The title shell now leads with one Start/Continue route and a promoted `3 Learn to Play` route; the shared Player Guide provides five beginner lessons plus an optional shortcut reference without requiring advanced keys
- `F1` opens the same paged guide during a run, while Pause exposes `F1 Player Guide` beside Resume, Save, Menu, Settings, and Quit; guide tabs, Left/Right, Previous/Next, and Close & Play all preserve the current run
...
- Compact-window layout guards now keep every beginner-guide lesson, page tab, Pause action, and deep-panel action clear of footer controls, while terminal onboarding guidance wraps complete sentences at narrow console widths
...
The preparation command selects the next uncovered featured campaign or exact unresolved active retest, allocates unused anonymous session/tester codes, prints the visible launch and observation checklist, and writes `/tmp/nexus-tech-beta-playtest-next.md`. Prepare only when the owner or tester is present and ready to complete the visible session. The packet embeds its own required preflight command and is bound to that output path; run the standalone validator immediately before the first launch, and regenerate rather than moving or editing the artifact. While current-version evidence remains `0/6`, the packet begins with a separate Owner Rehearsal Gate covering New Game, Guided Opening, Pause/Back/Menu/Continue recovery, both campaign choices, guided/full Endgame, Save & Archive, and the direct Progress handoff. Its recommended `run-beta-owner-rehearsal` workflow validates the packet, opens the exact 2D rehearsal profile, and runs the archive gate when the window closes; an incomplete close fails safely and the same guarded command can resume that rehearsal profile. The packet also retains the separate launch and post-rehearsal commands for diagnostics. The archive gate fails until the isolated rehearsal profile contains a complete two-choice archive for the exact target scenario and the tester profile is still unused. Rehearsal and tester launches use different fresh temporary gameplay databases, while recorder/status commands retain the persistent evidence database; this prevents prior Continue, save, or archive state from contaminating a first-time session. Packet output is also prohibited from sharing a path with any database or SQLite sidecar. Use `--session-db-path` or `--rehearsal-db-path` only for fresh explicit profiles because preparation rejects an existing profile. The rehearsal section has no recorder command and must never be entered as human evidence; the human recorder template remains in its separate post-session section. Generated beta, onboarding, and animation handoffs automatically reuse the `nexus-tech` executable that created them, so a `.venv313/bin/nexus-tech` launch no longer produces unusable `uv run` follow-up commands. Use `--command-prefix` only when the packet will run in another environment. Preparation never copies stored free-form notes, records a session, or treats an owner rehearsal, headless run, test, or screenshot as human evidence. Replace every ALL_CAPS field in its recorder template only after observing a real session.

Every active session packet defines `P0` release blockers, `P1` usability blockers, and `P2` polish defects with explicit stop and recording rules. This prevents crashes, data loss, and blocked recovery from being classified like non-blocking visual or copy issues.

Validate the generated packet immediately before owner rehearsal or tester handoff:

```bash
.venv313/bin/nexus-tech validate-beta-playtest-session-packet \
  --input /tmp/nexus-tech-beta-playtest-next.md
```

The same command is embedded inside the generated packet. The validator checks manifest schema 3 against the current build, intended packet path, and evidence store; requires the generated Markdown to remain unchanged; and confirms that every required gameplay profile is still unused and isolated. This standalone validator is a first-launch preflight and intentionally fails after a rehearsal profile has been created. Retry an incomplete rehearsal through `run-beta-owner-rehearsal` itself; regenerate instead of copying, moving, or editing a packet when first-launch validation fails. This consistency check does not create human evidence or replace reviewer approval.

Use the packet's guarded command for the complete visible owner workflow:

```bash
.venv313/bin/nexus-tech run-beta-owner-rehearsal \
  --input /tmp/nexus-tech-beta-playtest-next.md
```

It performs packet preflight, opens the exact packet-bound 2D profile, and checks the archive after the window closes. If the route is incomplete, rerun this guarded command directly; it accepts the existing rehearsal profile and directs you to New Game or Continue from the actual saved state. Regenerate only when the packet is stale or contaminated, or when the existing rehearsal profile is intentionally abandoned. Do not close the visible window until Save & Archive is complete and the target route is visible in Progress and Route Atlas. It never opens the tester profile or records human evidence.

For manual launch diagnostics, complete the visible owner route, use Save & Archive, and then run the embedded post-rehearsal command:

```bash
.venv313/bin/nexus-tech validate-beta-owner-rehearsal \
  --input /tmp/nexus-tech-beta-playtest-next.md
```

Both workflows revalidate the current packet and evidence snapshot, require the tester profile to remain fresh, and check the rehearsal database for the exact target scenario with both campaign choices archived. They intentionally cannot prove the visible Pause, Back, Menu, Continue, Endgame switching, or Route Atlas checks; the owner must still complete those checklist items. Neither workflow writes a human-session row.

Export the complete six-campaign queue without pre-allocating later testers:

```bash
.venv313/bin/nexus-tech beta-playtest-plan
```

The plan writes `/tmp/nexus-tech-beta-playtest-plan.md`, marks one lane as the next session or retest, keeps later lanes queued, and embeds the exact refresh and fail-closed commands. Packet and plan output paths are normalized before writing; they must be distinct and cannot alias the evidence database or its SQLite sidecars. Generate and validate only one session packet at a time, then regenerate the plan after recording one real observed session. The plan never copies free-form notes, creates gameplay profiles, records evidence, or approves release.
...
Before tagging, require every automated human-evidence criterion to be ready:
...
The default SQLite file is local and ignored by Git. Session keys must be unique unless `--replace` is supplied for a factual correction of the same observation. After fixing an observed gate failure, use the generated packet's `--retest-of` value with a new first-time tester; schema 28 retains the original row as history and evaluates only the active leaf of that retest chain. Retests must match the parent campaign and game version, cannot reuse the parent tester code, and cannot supersede a passing row. The status command requires six distinct active anonymous tester codes and all six featured campaigns, while still leaving the final release decision to a human reviewer. `--require-review-ready` returns a non-zero exit code until those criteria pass, so tag or release scripts fail closed; its success means ready for review, never automatic approval. The repository currently contains no human-session evidence, so the baseline remains `0/6` rather than an automated pass.
...
If you are new to the game, choose `3 Learn to Play` on the title screen or press `F1` during a run. The shared Player Guide starts with five beginner lessons covering the goal, one-turn loop, screen hierarchy, recovery, and the six controls needed for normal play. Its final shortcut reference is optional. You can also run `nexus-tech guide`, `nexus-tech tutorial`, or use the in-session Guided Opening panel to follow Coach Move, Spend AP, and End Turn without exposing internal command ids. Coach executes the current recommended move instead of opening a read-only hint.

For the animated frontend shell, run `nexus-tech play-2d --scenario founder_journey --seed 7` or open the full menu flow with `nexus-tech menu-2d`; add `--motion-mode reduced` or `--motion-mode off` if you want quieter highlight, entity, actor, action-feedback, and transition animation. From the title scene, a new player sees `Start Guided Game` beside `Learn to Play`; a returning player sees `Continue` beside `New Guided Game`, then a dedicated full-width guide route above the balanced two-column save, archive, progress, and settings grid. Quit remains visually separate from the learning and progression routes. Inside a run, `Choose This Turn` keeps one green Recommended route ahead of two non-duplicated alternatives, Report, Save, and End Turn; each action name stays primary while its fixed shortcut appears in a separate badge. You can click the Recommended Move and End Turn Check cards directly, plus product cards, modal options, deep-dive panel buttons, and the action bar, or use keys like `Tab`, `C`, `N`, `1`, `2`, `3`, `4`, `5`, `6`, `7`, `8`, `V`, `I`, `Q`, `F`, `M`, `D`, `H`, `A`, `O`, `Y`, `R`, `B`, `U`, `L`, `G`, `Space`, `S`, `P`, `Esc`, and `F1`. `V` toggles guided and full Endgame actions while that board is open. Endgame readiness values use a `/100` scale, and lower Reset Risk is safer. `P` opens Pause with Resume, Save, Menu, Settings, Player Guide, and Quit controls; `T` opens Settings while paused; and `Esc` returns from Settings to Pause before resuming the run. `M` saves and returns only when the run came from the title shell; direct play marks Menu unavailable and keeps `M` inert, while `Q` remains explicit quit. Action and impact cues use product names and metric labels rather than exposing internal identifiers. After a title-shell run uses `Save & Archive`, the completed review keeps Back/Menu available, separates each finding into Cause and Lesson, and adds `6 Open Progress` for a direct Route Atlas and next-route handoff. The shared responsive frame reserves navigation, header, content, and footer regions across title, run, review, and turn-summary scenes. The live run retains its shape-based entity motion, deterministic shape-sprite actors, command feedback, scene transitions, compact action copy, adaptive feed pacing, staged turn summaries, endgame cockpit, and remembered inspector controls. If a button is disabled, the 2D shell still tells you exactly which prerequisite is missing before you waste the click.

For a larger, high-contrast 2D profile, use Settings or append `--ui-scale large --contrast-mode high` to a 2D launch command. Settings changes persist in the local SQLite profile; keys `1` through `4` cycle text, contrast, motion, and action loadout. CLI flags override display settings for the current launch. The supported 2D viewport starts at 820x620; launch arguments below that contract are rejected and live resizing clamps to it so menu, gameplay, and modal lanes cannot collapse into one another. Retina and ultrawide windows use a centered 1440x900 maximum design canvas so panels, navigation, transient feedback, and modal controls retain the same readable proportions instead of stretching to the physical screen edges. Standard and spacious windows also use spare viewport room for larger typography automatically, including after live resizing. When no save exists, Title shows Start Guided Game and Learn to Play instead of a disabled Continue route; returning players regain Continue as the primary route. Their compact header reserves separate subtitle and archive-progress lanes so status copy stays clear of the panel frame. Title Menu uses equal primary columns plus a balanced responsive utility grid, while Turn Summary and Review keep each contextual action in one visible owner region instead of repeating the same click target in both navigation and content. Blocking run states now select one visible overlay owner, discard hidden workspace targets, and keep scene transitions below dialogs so picker, inspector, Help, Pause, Settings, outcome, and text-entry controls remain readable and cannot click through. The live run still reserves its top rail for Pause, Back, More, and Help recovery routes. The Report inspector and terminal report include a Decision Ledger with immediate impact, end-turn follow-on timing, and a derived Decision Pattern. Compact Focus View arranges its six preserved actions in two balanced rows, while wider layouts keep one six-action row. Spacious Focus cards promote their decision headlines and details, and blocking overlays keep background scene copy visually subordinate. Turn Summary uses responsive phase labels and a bounded one-line action footer instead of shrinking long labels inside fixed chips. The decision preview separates the recommended move, AP cost, remaining AP, timing, expected outcome, and skip risk into three bounded lines; hover adds consequence context, and Recommended disables when a one-AP move is no longer affordable. First-turn header and checkpoint instructions switch to complete short forms when their measured lane is narrow. Arbitrary key remapping is not included in the current release; the documented keyboard bindings remain fixed.

Compact buttons never expose a clipped helper sentence: they use complete authored short copy when it fits and otherwise keep a clean title-only card, while hover preserves the full explanation. The visual audit covers all 39 menu, gameplay, modal, deep-panel, inspector, summary, and review surfaces at every supported text scale.
