...
- Lightweight `pygame-ce` 2D dashboard frontend with animated bars, product cards, action/context pickers, clickable controls, title/save-load scenes, deep-dive overlays, and review scenes
...
- Built-in `play-2d`, `load-game-2d`, `continue-last-game-2d`, and `menu-2d` commands for the first animated frontend shell, now with clickable action bars, save/archive browsing, and post-turn summaries
...
For the first animated frontend shell, run `nexus-tech play-2d --scenario founder_journey --seed 7` or open the menu flow with `nexus-tech menu-2d`. You can click product cards, coach cards, modal options, deep-dive panel buttons, and the action bar directly, or use keys like `Tab`, `C`, `N`, `1`, `2`, `3`, `4`, `Q`, `F`, `M`, `D`, `H`, `A`, `Y`, `R`, `B`, `U`, `L`, `G`, `Space`, and `S`.
