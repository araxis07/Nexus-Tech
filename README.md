...
After completing the packet's visible owner route and using Save & Archive, run its embedded post-rehearsal command:
...
This gate revalidates the current packet and evidence snapshot, requires the tester profile to remain fresh, and checks the rehearsal database for the exact target scenario with both campaign choices archived. It intentionally cannot prove the visible Pause, Back, Menu, Continue, Endgame switching, or Route Atlas checks; the owner must still complete those checklist items. It never writes a human-session row.
