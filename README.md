...
- Contextual 2D action footer keeps the core play loop visible, adds Coach-recommended commands as needed, and reveals partner, board, and endgame routes when their phase or pressure makes them relevant
- Player-facing command metadata groups all 194 turn commands into 12 readable action families, while the HUD explains the current Opening, Growth, Scale, or Endgame phase without exposing raw command ids
- The new-game wizard leads with six featured tracks for learning, profit, quality recovery, portfolio scale, debt pressure, and endgame readiness before the specialist challenge catalog
