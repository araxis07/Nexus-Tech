...
- Every 2D launch path supports `--ui-scale compact|standard|large` and `--contrast-mode standard|high` alongside full/reduced/off motion profiles
...
For a larger, high-contrast 2D profile, append `--ui-scale large --contrast-mode high` to `play-2d`, `load-game-2d`, `continue-last-game-2d`, or `menu-2d`. Arbitrary key remapping is not included in 0.283.0; the documented keyboard bindings remain fixed for this release.
