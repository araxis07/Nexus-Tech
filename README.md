...
The same command is embedded inside the generated packet. The validator checks manifest schema 3 against the current build, intended packet path, and evidence store; requires the generated Markdown to remain unchanged; and confirms that every required gameplay profile is still unused and isolated. Regenerate instead of copying, moving, or editing a packet when validation fails. This consistency check does not create human evidence or replace reviewer approval.
...
It performs packet preflight, opens the exact packet-bound 2D profile, and checks the archive after the window closes. If the route is incomplete, rerun the same command; it directs you to New Game or Continue from the actual saved state. It never opens the tester profile or records human evidence.
