...
- Lightweight `pygame-ce` 2D dashboard frontend with animated bars, product cards, action/context pickers, clickable controls, and a turn-summary scene
...
- Built-in `play-2d`, `load-game-2d`, and `continue-last-game-2d` commands for the first animated frontend shell, now with clickable action bars and post-turn summaries
...
For the first animated frontend shell, run `nexus-tech play-2d --scenario founder_journey --seed 7`. You can click product cards, coach cards, modal options, and the action bar directly, or use keys like `Tab`, `C`, `Q`, `F`, `M`, `D`, `H`, `A`, `Y`, `R`, `B`, `U`, `L`, `G`, `Space`, and `S`.
