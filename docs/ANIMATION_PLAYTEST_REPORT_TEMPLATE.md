...
| `audit-2d-animation` |  | Confirm `Actor State Coverage`, `Readability Guard`, `Scene Motion Profile`, `Animation Pacing Budget`, `Motion Mode Differentiation`, `Long Session Motion Stress`, `Long Session Visual Readiness`, `Visual Fatigue Budget`, and `actor-readability` pass |
