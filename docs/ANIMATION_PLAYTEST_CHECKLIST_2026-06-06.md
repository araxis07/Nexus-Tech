...
- Local launcher guard: add `--command-prefix .venv313/bin/nexus-tech` to the queue, validation, plan, and session commands when the manual tester is running the project from the local virtualenv instead of `uv run nexus-tech`.
...
- Playtest prep guard: `prepare-2d-animation-playtest --matrix-input /tmp/nexus-tech-animation-matrix.md --output /tmp/nexus-tech-animation-playtest-prep.md` writes the window/motion checklist from the validated matrix evidence before the human pass starts without rerunning the matrix.
...
- Local launcher guard: use the same `--command-prefix .venv313/bin/nexus-tech` value across queue generation, validation, planning, and next-action checks when the tester cannot run `uv`.
