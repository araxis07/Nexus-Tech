...
NEXUS TECH 0.315.0 is a late-alpha beta candidate with a complete vertical slice. The six featured campaigns have real three-act progression, and every combination of their Commitment and Consequence decisions now has deterministic native-goal evidence across all three difficulties. Archived endings derive discovery and mastery across all 24 authored routes, while live and archived reviews synthesize both choices into a Campaign Legacy. A save-compatible six-step First Archive Mission connects Guided Opening, both campaign decisions, Endgame, the final outcome, and Save & Archive across terminal and 2D play. Terminal and 2D progression show the complete six-campaign Route Atlas. Strategic Rhythm joins campaign objective, Quarter Plan progress, current move, end-turn check, and delayed follow-on without adding state. Decision Pattern derives voluntary operating-family mix, unique-choice coverage, and repetition signals from the existing Decision Ledger while excluding forced event responses. A deterministic 18-cell decision-quality matrix now checks ledger coverage and exports advisory repetition or low-variety candidates without treating autoplay as player evidence. Human beta preparation converts the current coverage gap into isolated rehearsal and tester launches, a checklist, recorder template, and local handoff without writing evidence. While evidence remains `0/6`, the packet opens with a separate owner-only rehearsal gate covering recovery, campaign, Endgame, archive, and Progress routes without creating a rehearsal recorder; the human recorder remains in its separate post-session section. Owner rehearsal, tester gameplay, and structured evidence use three distinct database paths so prior Continue, save, and archive state cannot contaminate a first-time observation. Generated human-beta, onboarding visible-window, and animation manual-QA handoffs reuse the active `nexus-tech` executable instead of assuming `uv` exists, while explicit cross-environment overrides remain supported. Pause now differentiates title-shell return from direct play and prevents unavailable `M` input from closing an unsaved direct run. Title Menu and Quick Start emphasize New Game when no save exists and present Continue as an unavailable returning-player route instead of a competing primary action. A pure scene-chrome policy keeps Title Menu, Run, Turn Summary, and Review action ownership mutually exclusive across content, footer, navigation, and blocking overlays while live-run recovery controls remain in their persistent top rail. Compact title headers keep subtitle and archive-progress copy in separate measured lanes above the panel frame. The Focus View footer presents one emphasized `Recommended` route, two non-duplicated alternatives, Report, Save, and End Turn in a balanced compact or wide grid. Its measured decision preview now separates the recommended move, AP cost and remainder, timing, expected outcome, and skip risk across three complete lines; Recommended disables when the shared AP policy says the move is no longer affordable. Guided Opening uses one `NEXT` then `LATER` hierarchy instead of repeating navigation shortcuts across every surface. First-turn visibility, checkpoint progress, semantic tones, and responsive copy now come from pure presentation policy; narrow header and card lanes use complete short forms instead of renderer ellipses. The 2D Endgame Board reveals its recommended fix and main risk before advanced paths, reserves a compact header lane for actor cards, and keeps those cards clear of title and summary copy. Guided Endgame instructions and responsive Review findings preserve complete compact and large-text meaning without ellipses. Quick Start identifies its title actor as `Guide`, compact Focus footer copy preserves complete decision cues, and the completed-run overlay explains the ending with a Cash/Score/Turn snapshot before Review and Save & Archive. Archived reviews reached from the title shell preserve Back/Menu and expose a direct `6 Open Progress` handoff into Route Atlas. Pause, Focus, decision preview, action-bar, completed-run, Review navigation and presentation, Help, title hierarchy, scene chrome, transient scene state, workspace-routing policy, and owner-rehearsal policy now live in focused modules outside the scene renderer, with recovery, catalog, first-turn progress, focus hierarchy, command ownership, campaign identity, shutdown, victory, archive, and large-text contracts covered directly. Schema 27 stores decision history additively, keeps display/loadout preferences outside gameplay save slots, and retains isolated structured human-session evidence.
...
- Human-beta, onboarding visible-window, and animation manual-QA packets now derive their default command prefix from the running `nexus-tech` executable.
- Project-local absolute launchers become shell-safe relative paths, explicit `--command-prefix` values remain unchanged, and generated packets no longer fail merely because `uv` is absent from the operator shell.
- Direct-play `M` no longer closes the 2D shell through an unavailable Menu route; the paused run remains open and `Q` remains explicit quit.
- Pause guidance and Menu availability now derive from whether the run has a title-shell return destination, with disabled routes omitted from mouse targets.
- Pure `frontend_2d.pause_presentation` policy owns pause status, guidance, action order, tone, and availability without saving or mutating the run.
- Action and impact feedback use stable product and metric labels rather than exposing internal UUIDs, keeping cue copy readable and seeded visual output reproducible.
- Title Menu and Quick Start now give New Game the only primary emphasis when no save exists, while Continue uses disabled styling and a complete unavailability reason.
- Returning-player title flows preserve Continue as the primary route, New Game as an alternative, and every established numeric shortcut.
- Pure `frontend_2d.title_presentation` policy owns title action availability, tone, copy, footer guidance, and measured header lanes without loading, mutating, or rendering a run.
- Compact Focus View now presents all six preserved actions in a symmetric `3x2` grid instead of a five-button row plus a detached End Turn control; wide layouts use one `6x1` row.
- Recommended keeps visible primary emphasis with motion disabled, while the two alternatives, Report, Save, End Turn, and every underlying direct command remain unchanged.
- Guided Opening header, card, and compact footer copy now explain the `NEXT` then `LATER` sequence without repeating Pause, Save, and End Turn shortcuts already visible in navigation and actions.
- Pure `frontend_2d.focus_presentation` policy owns opening copy, Focus grid columns, and primary emphasis without changing simulation or persistence.
- Guided Endgame cards now use complete compact fix and risk instructions rather than pre-truncating every detail at 32 characters.
- Responsive Review finding cards allocate two-line cause and lesson budgets, reduce visible density for the 820x620 large-text profile, and keep the larger-layout notice outside card bounds.
- Pure `frontend_2d.review_presentation` policy owns the responsive card budget while the renderer retains geometry and the postmortem retains full findings.
- Completed reviews now add `6 Open Progress` after `Save & Archive` while retaining the existing Back/Menu action and direct-launch close behavior.
- Pure `frontend_2d.review_navigation` policy owns post-run action order, copy, and availability for both the footer and navigation rail.
- Title, run, and turn-summary flows carry an optional Progress destination through the completed-review handoff without adding save state.
- Campaign identity contracts require unique authored chapter language, distinct option mechanics, and at least ten affected state dimensions per featured campaign.
- Transient title, run, review, summary, input, cue, and actor records now live in `frontend_2d.scene_state` while `scenes.py` preserves compatibility imports for runtime audits.
- One pure workspace-routing policy now owns command destinations for navigation, action feedback, and late-game choreography instead of duplicating seven command families inside the scene renderer.
- A persisted completed-campaign regression verifies that archive creation closes the six-step First Archive Mission; it is an automated owner-flow contract, not evidence of visible play or human comprehension.
- Focus View now labels its first decision route `Recommended`, gives it primary visual emphasis, and excludes the same command or workspace from the two alternative slots.
- The pure action-bar policy preserves player loadout preference, Coach ranking, stable fallbacks, Report, Save, End Turn, and every underlying direct control without changing simulation behavior.
- Endgame actor cards occupy one measured compact-header lane, title and summary copy reserve that width, and the redundant entity strip is hidden while actors are active.
- Quick Start labels its title actor as `Guide`, with direct role and 820x620 overlap regressions.
- The 25-button run action catalog and Contextual, Product, Growth, and Resilience loadout policies now live outside `scenes.py`, with direct key-order and payload contracts and no control changes.
...
- The first current-version packet begins with an owner-only rehearsal checklist and complete folded evidence boundary; it disappears after a real current-version session exists.
...
- `decision-quality-audit` runs the six featured campaigns across Builder, Standard, and Founder, fails missing operating-ledger coverage, and exports repetition or low-variety watches as human-review candidates only.
...
- Endgame progressive disclosure keeps Recommended Fix and Review Main Risk visible first; `V` or the local More control reveals all existing path fixes and reviews without removing command routes.
- Compact Focus footer copy now shows a complete Next, End Turn, Later, and Coach-hover handoff instead of truncating the rationale lane.
- The pure `frontend_2d.panel_disclosure` module owns guided versus expanded panel policy while the scene renderer remains responsible for geometry and input routing.
...
Automated coverage verifies campaign boundaries and delayed effects, event priority, all 72 campaign-route/difficulty cells, early-victory prevention, path retention, route-mastery derivation, Campaign Legacy synthesis, all six First Archive Mission states, archive confirmation and duplicate-save prevention, additive migration through schema 28, Decision Ledger round trips, deterministic decision-quality ledger coverage, action-loadout persistence and prioritization, Strategic Rhythm plan/follow-on derivation, opening-versus-endgame recommendation order, archive evidence, structured-session validation and retest lineage, preference persistence/fallback, the 6-by-3 balance matrix, the long-session Debt Crunch envelope, catalog ceilings, Focus View layout, Settings typography, Pause recovery, accessibility-profile launch, and responsive containment. Decision-quality watches remain advisory and cannot complete a human gate or authorize tuning.
...
Arbitrary key remapping is not part of 0.315.0. Keyboard-only control remains available through the documented bindings, but remapping and assistive-technology compatibility require separate design and real-device validation.

## Next Steps

1. Generate the next-session packet, preserve its separate fresh rehearsal and tester gameplay profiles plus persistent evidence store, then follow its Owner Rehearsal Gate from New Game through `Save & Archive`; verify compact footer comprehension, guided/full Endgame switching, and the direct `6 Open Progress` replay handoff, record only defects actually observed in the real window, and never use the human-session recorder for that rehearsal.
...
4. Compare real player choice comprehension and pacing against Campaign Legacy, route-mastery guidance, and decision-quality watch candidates; tune only where observed behavior confirms the same problem.
