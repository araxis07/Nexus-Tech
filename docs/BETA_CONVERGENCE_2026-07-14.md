...
- Pure `frontend_2d.title_presentation` policy owns title action availability, tone, copy, and footer guidance without loading or mutating a run.
...
Arbitrary key remapping is not part of 0.309.0. Keyboard-only control remains available through the documented bindings, but remapping and assistive-technology compatibility require separate design and real-device validation.
