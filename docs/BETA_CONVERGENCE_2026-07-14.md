...
Arbitrary key remapping is not part of 0.311.0. Keyboard-only control remains available through the documented bindings, but remapping and assistive-technology compatibility require separate design and real-device validation.

## Next Steps

1. Generate the next-session packet, then follow its Owner Rehearsal Gate from New Game through `Save & Archive`; verify compact footer comprehension, guided/full Endgame switching, and the direct `6 Open Progress` replay handoff, record only defects actually observed in the real window, and never use the human-session recorder for that rehearsal.
