...
Use `animation-playtest-progress` when the tester needs a completion percentage,
open manual lanes, and the same next manual action in one board. The progress
board is advisory only; it does not record or replace real tester evidence.
Use `animation-playtest-execution-guide` when the tester needs one operator
artifact that pairs every open visible command with the required terms, evidence
prompt, and recorder command. The guide still requires real observations before
any recorder command is valid.
Use `animation-playtest-issue-backlog` after report edits to turn fail/watch
rows and missing evidence into a validated P0/P1/P2 queue for the next UI or
animation fix pass.
Use `animation-playtest-sprint` when the tester needs one focused work packet
...
use the batch evidence checklist to decide pass/watch/fail and replace the
recorder placeholders with notes from the visible game window. After recording
the batch, run the post-recording refresh commands in the same artifact to
regenerate route batches, validate them, and check the current report status.
