...
def test_new_event_ids_are_registered() -> None:
    ...
    assert "ipo_pricing_guardrail" in registry_ids
    assert "ipo_order_book_covenant" in registry_ids
    assert "ipo_book_stability_panel" in registry_ids
    assert "ipo_demand_floor" in registry_ids
    ...
    assert "ipo_book_anchor_statute" in registry_ids
    assert "ipo_book_anchor_mandate" in registry_ids
    assert "ipo_book_anchor_commission" in registry_ids
    ...
    assert "buyer_close_committee" in registry_ids
    assert "buyer_close_signoff" in registry_ids
    assert "buyer_close_war_room" in registry_ids
    assert "buyer_synergy_holdback" in registry_ids
    ...
    assert "buyer_close_anchor_statute" in registry_ids
    assert "buyer_close_anchor_mandate" in registry_ids
    assert "buyer_close_anchor_commission" in registry_ids
    ...
    assert "independence_liquidity_grid" in registry_ids
    assert "independence_reserve_compact" in registry_ids
    assert "independence_treasury_lattice" in registry_ids
    assert "independence_cash_conversion_pact" in registry_ids
    ...
    assert "independence_cash_solvency_statute" in registry_ids
    assert "independence_cash_solvency_mandate" in registry_ids
    assert "independence_cash_solvency_commission" in registry_ids
    ...
    assert "reseller_service_council" in registry_ids
    assert "reseller_service_charter" in registry_ids
    assert "reseller_reference_scorecard" in registry_ids
    assert "reseller_margin_reconciliation" in registry_ids
    ...
    assert "reseller_service_statute" in registry_ids
    assert "reseller_service_mandate" in registry_ids
    assert "reseller_service_commission" in registry_ids
    ...
    assert "integration_hypercare_grid" in registry_ids
    assert "integration_hypercare_board" in registry_ids
    assert "integration_hypercare_sla" in registry_ids
    assert "integration_reliability_bridge" in registry_ids
    ...
    assert "integration_cutover_statute" in registry_ids
    assert "integration_cutover_mandate" in registry_ids
    assert "integration_cutover_commission" in registry_ids
    ...
    assert "marketplace_refund_bench" in registry_ids
    assert "marketplace_refund_appeal" in registry_ids
    assert "marketplace_refund_arbitration" in registry_ids
    assert "marketplace_dispute_escrow" in registry_ids
    ...
    assert "marketplace_refund_statute" in registry_ids
    assert "marketplace_refund_mandate" in registry_ids
    assert "marketplace_refund_commission" in registry_ids
    ...
    assert "board_reset_runway_table" in registry_ids
    assert "board_reset_operating_charter" in registry_ids
    assert "board_reset_execution_council" in registry_ids
    assert "board_reset_resilience_statute" in registry_ids
    ...
    assert "board_reset_operating_statute" in registry_ids
    assert "board_reset_operating_mandate" in registry_ids
    assert "board_reset_operating_commission" in registry_ids
...
def test_run_enterprise_commitment_board_hardens_flagship_exit_story() -> None:
    product = make_product(
        "Enterprise Commitment Board Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Commitment Board Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("6400.00"),
        support_tier=SupportTier.PRIORITY,
        satisfaction=50,
        onboarding_health=46,
        support_load=36,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("11600.00"))
    state.support_program.backlog_queue = 12
    state.support_program.escalation_queue = 5
    state.finance.board_pressure = 30
    state.finance.board_confidence = 34
    state.finance.board_score = 32
    state.finance.investor_pressure = 18
    state.company.reputation = 70
...
def test_run_enterprise_reference_chamber_rebuilds_flagship_proof_stack() -> None:
    product = make_product(
        "Enterprise Reference Chamber Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Reference Chamber Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("14800.00"))
    state.support_program.backlog_queue = 14
    state.support_program.escalation_queue = 6
    state.finance.board_pressure = 32
    state.finance.board_confidence = 32
...
def test_run_enterprise_reference_forum_rebuilds_flagship_exit_proof() -> None:
    product = make_product(
        "Enterprise Reference Forum Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Reference Forum Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("8400.00"),
        support_tier=SupportTier.WHITE_GLOVE,
        satisfaction=44,
        onboarding_health=38,
        support_load=42,
        ...
        churn_risk=40,
        invoice_risk=28,
        failed_payment_risk=22,
        escalation_count=4,
        status=CustomerAccountStatus.AT_RISK,
    )
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("19200.00"))
    ...
    state.finance.investor_pressure = 22
    state.company.reputation = 66
...
def test_run_enterprise_reference_lattice_rebuilds_terminal_flagship_exit_proof() -> None:
    product = make_product(
        "Enterprise Reference Lattice Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Reference Lattice Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("9200.00"),
        support_tier=SupportTier.WHITE_GLOVE,
        satisfaction=40,
        ...
        churn_risk=42,
        invoice_risk=30,
        failed_payment_risk=24,
        escalation_count=5,
        status=CustomerAccountStatus.AT_RISK,
    )
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("26000.00"))
    state.support_program.backlog_queue = 18
...
def test_run_enterprise_reference_summit_rebuilds_terminal_flagship_exit_proof() -> None:
    product = make_product(
        "Enterprise Reference Summit Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Reference Summit Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("9800.00"),
        support_tier=SupportTier.WHITE_GLOVE,
        satisfaction=34,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("36400.00"))
    state.support_program.backlog_queue = 22
...
def test_run_billing_collection_bridge_cools_extreme_collections_heat() -> None:
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("11200.00"))
    state.support_program.backlog_queue = 9
    state.support_program.escalation_queue = 4
...
def test_run_billing_collection_office_cools_deep_collections_and_covenant_heat() -> None:
    product = make_product("Billing Collection Office Core", target_segment=MarketSegment.SMB)
    account = CustomerAccount(
        name="Collection Office Anchor",
        product_id=product.id,
        segment=MarketSegment.SMB,
        ...
        churn_risk=38,
        invoice_risk=58,
        failed_payment_risk=52,
        dunning_steps=5,
        escalation_count=4,
        status=CustomerAccountStatus.AT_RISK,
    )
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("15600.00"))
    state.support_program.backlog_queue = 11
    state.support_program.escalation_queue = 5
    state.finance.board_pressure = 30
    state.finance.investor_pressure = 24
...
def test_run_billing_settlement_board_cools_deep_collections_and_covenant_heat() -> None:
    product = make_product("Billing Settlement Board Core", target_segment=MarketSegment.SMB)
    account = CustomerAccount(
        name="Settlement Board Anchor",
        product_id=product.id,
        segment=MarketSegment.SMB,
        ...
        churn_risk=40,
        invoice_risk=62,
        failed_payment_risk=56,
        dunning_steps=6,
        escalation_count=4,
        status=CustomerAccountStatus.AT_RISK,
    )
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("19600.00"))
    state.support_program.backlog_queue = 12
    state.support_program.escalation_queue = 6
    state.finance.board_pressure = 32
    state.finance.investor_pressure = 26
...
def test_run_billing_cash_war_room_cools_terminal_collections_and_covenant_heat() -> None:
    product = make_product("Billing Cash War Room Core", target_segment=MarketSegment.SMB)
    account = CustomerAccount(
        name="Cash War Room Anchor",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("4200.00"),
        support_tier=SupportTier.PRIORITY,
        ...
        churn_risk=44,
        invoice_risk=68,
        failed_payment_risk=60,
        dunning_steps=7,
        escalation_count=5,
        status=CustomerAccountStatus.AT_RISK,
    )
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("24400.00"))
    state.support_program.backlog_queue = 14
    state.support_program.escalation_queue = 7
    state.finance.board_pressure = 34
    state.finance.investor_pressure = 28
...
def test_run_billing_liquidity_command_cools_terminal_collections_and_covenant_heat() -> None:
    product = make_product("Billing Liquidity Command Core", target_segment=MarketSegment.SMB)
    account = CustomerAccount(
        name="Liquidity Command Anchor",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("4600.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("29600.00"))
    state.support_program.backlog_queue = 16
    state.support_program.escalation_queue = 8
    state.finance.board_pressure = 36
    state.finance.investor_pressure = 30
    state.finance.covenant_risk = 28
    state.finance.board_confidence = 26
...
def test_run_billing_liquidity_summit_cools_terminal_collections_and_covenant_heat() -> None:
    product = make_product("Billing Liquidity Summit Core", target_segment=MarketSegment.SMB)
    account = CustomerAccount(
        name="Liquidity Summit Anchor",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("5200.00"),
        support_tier=SupportTier.PRIORITY,
        ...
        sla_breach_risk=60,
        ticket_queue_age=6,
        expansion_potential=36,
        renewal_health=22,
        renewal_turn=4,
        churn_risk=48,
        invoice_risk=76,
        failed_payment_risk=68,
        dunning_steps=9,
        escalation_count=6,
        status=CustomerAccountStatus.AT_RISK,
    )
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("35600.00"))
    state.support_program.backlog_queue = 18
    state.support_program.escalation_queue = 9
    state.finance.board_pressure = 38
    state.finance.investor_pressure = 32
    state.finance.covenant_risk = 30
    state.finance.board_confidence = 24
...
def test_run_onboarding_stability_board_hardens_extreme_implementation_recovery() -> None:
    product = make_product(
        "Onboarding Stability Board Core", target_segment=MarketSegment.ENTERPRISE
    )
    account = CustomerAccount(
        name="Stability Board Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3600.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("14600.00"))
    state.support_program.backlog_queue = 12
    state.support_program.escalation_queue = 5
...
def test_run_onboarding_retention_mesh_hardens_extreme_implementation_recovery() -> None:
    product = make_product(
        "Onboarding Retention Mesh Core", target_segment=MarketSegment.ENTERPRISE
    )
    account = CustomerAccount(
        name="Retention Mesh Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("4000.00"),
        support_tier=SupportTier.WHITE_GLOVE,
        satisfaction=42,
        onboarding_health=18,
        support_load=40,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("15200.00"))
    state.support_program.backlog_queue = 12
    state.support_program.escalation_queue = 5
    state.finance.board_pressure = 28
...
def test_run_onboarding_assurance_grid_hardens_terminal_implementation_recovery() -> None:
    product = make_product(
        "Onboarding Assurance Grid Core", target_segment=MarketSegment.ENTERPRISE
    )
    account = CustomerAccount(
        name="Assurance Grid Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("18800.00"))
    state.support_program.backlog_queue = 15
    state.support_program.escalation_queue = 7
    state.finance.board_pressure = 30
...
def test_run_onboarding_durability_mesh_hardens_terminal_implementation_recovery() -> None:
    product = make_product(
        "Onboarding Durability Mesh Core", target_segment=MarketSegment.ENTERPRISE
    )
    account = CustomerAccount(
        name="Durability Mesh Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("4800.00"),
        support_tier=SupportTier.WHITE_GLOVE,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("24400.00"))
    state.support_program.backlog_queue = 17
    state.support_program.escalation_queue = 8
    state.finance.board_pressure = 32
    state.finance.board_confidence = 34
    state.company.reputation = 60
...
def test_run_onboarding_continuity_lattice_hardens_terminal_implementation_recovery() -> None:
    product = make_product(
        "Onboarding Continuity Lattice Core", target_segment=MarketSegment.ENTERPRISE
    )
    account = CustomerAccount(
        name="Continuity Lattice Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("5200.00"),
        support_tier=SupportTier.WHITE_GLOVE,
        satisfaction=34,
        onboarding_health=12,
        support_load=50,
        open_tickets=12,
        sla_breach_risk=58,
        ticket_queue_age=6,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("32800.00"))
    state.support_program.backlog_queue = 19
    state.support_program.escalation_queue = 9
    state.finance.board_pressure = 34
    state.finance.board_confidence = 32
    state.company.reputation = 58
...
def test_run_white_glove_reference_bureau_hardens_flagship_exit_readiness() -> None:
    product = make_product(
        "White Glove Reference Bureau Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Reference Bureau Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("10400.00"))
    state.support_program.backlog_queue = 12
    state.support_program.escalation_queue = 5
    state.finance.board_pressure = 30
    state.finance.board_confidence = 36
...
def test_run_white_glove_reference_exchange_rebuilds_terminal_flagship_account() -> None:
    product = make_product(
        "White Glove Reference Exchange Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Reference Exchange Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("6800.00"),
        support_tier=SupportTier.WHITE_GLOVE,
        satisfaction=48,
        onboarding_health=44,
        support_load=44,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("17600.00"))
    state.support_program.backlog_queue = 14
    state.support_program.escalation_queue = 6
    state.finance.board_pressure = 34
    state.finance.board_confidence = 28
...
def test_run_billing_dispute_cabinet_cools_extreme_billing_capital_heat() -> None:
    product = make_product("Billing Dispute Cabinet Core", target_segment=MarketSegment.SMB)
    account = CustomerAccount(
        name="Dispute Cabinet Anchor",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2600.00"),
        support_tier=SupportTier.PRIORITY,
...
def test_run_onboarding_launch_cell_stabilizes_multi_account_drag() -> None:
    product = make_product("Onboarding Launch Cell Core", target_segment=MarketSegment.ENTERPRISE)
    account = CustomerAccount(
        name="Launch Cell Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("2800.00"),
        support_tier=SupportTier.PRIORITY,
...
def test_finance_planner_recommends_enterprise_commitment_board_for_extreme_exit_heat() -> None:
    state = make_state(
        make_product("Enterprise Commitment Planning Core"),
        cash_on_hand=Decimal("5000.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    planner = build_finance_planner(
        ...
        support_backlog=12,
        support_escalations=4,
        revenue_at_risk_value=Decimal("2800.00"),
    ...
    assert "run_enterprise_commitment_board" in planner.recommended_actions
    assert any("enterprise commitment board" in step for step in planner.action_sequence)


def test_finance_planner_recommends_enterprise_reference_forum_for_flagship_heat() -> None:
    state = make_state(
        make_product("Enterprise Reference Forum Planning Core"),
        cash_on_hand=Decimal("5200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    planner = build_finance_planner(
        ...
        support_backlog=14,
        support_escalations=5,
        revenue_at_risk_value=Decimal("3600.00"),
    ...
    assert "run_enterprise_reference_forum" in planner.recommended_actions
    assert any("enterprise reference forum" in step for step in planner.action_sequence)
...
def test_finance_planner_recommends_channel_dependency_reset_for_extreme_hotspot_drag() -> None:
    state = make_state(
        make_product("Channel Dependency Planning Core"),
        cash_on_hand=Decimal("5000.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        focus_alignment_gap=2,
        channel_conflict_index=31,
        paused_dependency_score=60,
        hotspot_dependency_score=84,
        hotspot_revenue_share_percent=44,
        volatile_revenue_share_percent=27,
        hotspot_channel="integration",
        strategic_outlook="strategic_acquisition",
        dominant_endgame_pressure="acquirer_diligence",
        commercial_fragility=70,
    ...
    assert "run_channel_dependency_reset" in planner.recommended_actions
    assert any("channel dependency reset" in step for step in planner.action_sequence)


def test_finance_planner_recommends_channel_durability_mesh_for_extreme_channel_drag() -> None:
    state = make_state(
        make_product("Channel Durability Mesh Planning Core"),
        cash_on_hand=Decimal("5200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        hotspot_lane_account_count=2,
        focus_alignment_gap=2,
        channel_conflict_index=36,
        paused_dependency_score=64,
        hotspot_dependency_score=88,
        hotspot_revenue_share_percent=47,
        volatile_revenue_share_percent=30,
        hotspot_channel="integration",
        strategic_outlook="strategic_acquisition",
        dominant_endgame_pressure="acquirer_diligence",
        commercial_fragility=74,
    ...
    assert "run_channel_durability_mesh" in planner.recommended_actions
    assert any("channel durability mesh" in step for step in planner.action_sequence)


def test_finance_planner_recommends_billing_collection_bridge_for_extreme_covenant_heat() -> None:
    state = make_state(
        make_product("Billing Collection Planning Core"),
        cash_on_hand=Decimal("4200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        hotspot_lane_account_count=2,
        renewal_queue_risk_accounts=2,
        renewal_pressure_value=Decimal("3300.00"),
        strategic_outlook="profitable_independence",
        dominant_endgame_pressure="independence_discipline",
        commercial_fragility=64,
    ...
    assert "run_billing_collection_bridge" in planner.recommended_actions
    assert any("billing collection bridge" in step for step in planner.action_sequence)


def test_finance_planner_recommends_onboarding_adoption_hub_for_lane_overflow() -> None:
    state = make_state(
        make_product("Onboarding Adoption Planning Core"),
        cash_on_hand=Decimal("4700.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
            reserve_target=Decimal("5000.00"),
            product_investment_share=35,
            go_to_market_share=39,
            reserve_share=26,
        ),
    )
    planner = build_finance_planner(
        ...
        support_backlog=16,
        support_escalations=5,
        revenue_at_risk_value=Decimal("2700.00"),
        renewal_pressure_value=Decimal("2300.00"),
        support_lane_focus=SupportLaneFocus.BALANCED,
        support_hotspot_lane=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane_overflow=2,
    ...
    assert "run_onboarding_adoption_hub" in planner.recommended_actions
    assert any("onboarding adoption hub" in step for step in planner.action_sequence)
...
def test_finance_planner_recommends_white_glove_reference_bureau_for_extreme_exit_heat() -> None:
    state = make_state(
        make_product("White Glove Bureau Planning Core"),
        cash_on_hand=Decimal("5600.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
            reserve_target=Decimal("5900.00"),
            product_investment_share=34,
            go_to_market_share=37,
            reserve_share=29,
        ),
    ...
    planner = build_finance_planner(
        ...
        support_escalations=4,
        high_value_risk_accounts=2,
        premium_revenue_at_risk_value=Decimal("3400.00"),
        white_glove_queue_risk_accounts=2,
        premium_queue_risk_accounts=2,
        renewal_queue_risk_accounts=2,
        renewal_pressure_value=Decimal("2200.00"),
        ...
        strategic_outlook="ipo_ready",
        dominant_endgame_pressure="public_market_scrutiny",
        commercial_fragility=76,
        capital_fragility=60,
    )

    assert "run_white_glove_reference_bureau" in planner.recommended_actions
    assert any("white-glove reference bureau" in step for step in planner.action_sequence)


def test_finance_planner_recommends_white_glove_reference_exchange_for_flagship_heat() -> None:
    state = make_state(
        make_product("White Glove Reference Exchange Planning Core"),
        cash_on_hand=Decimal("5400.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    planner = build_finance_planner(
        ...
        support_escalations=5,
        high_value_risk_accounts=1,
        premium_revenue_at_risk_value=Decimal("3800.00"),
        revenue_at_risk_value=Decimal("4200.00"),
        white_glove_queue_risk_accounts=2,
        premium_queue_risk_accounts=2,
        enterprise_queue_risk_accounts=2,
    ...
    assert "run_white_glove_reference_exchange" in planner.recommended_actions
    assert any("reference exchange" in step for step in planner.action_sequence)


def test_finance_planner_recommends_enterprise_reference_lattice_for_terminal_flagship_heat() -> (
    None
):
    state = make_state(
        make_product("Enterprise Reference Lattice Planning Core"),
        cash_on_hand=Decimal("5200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-880.00"),
        capital_plan=state.capital_plan,
        support_backlog=18,
        support_escalations=6,
        high_value_risk_accounts=1,
        premium_revenue_at_risk_value=Decimal("4600.00"),
    ...
    assert "run_enterprise_reference_lattice" in planner.recommended_actions
    assert any("enterprise reference lattice" in step for step in planner.action_sequence)


def test_finance_planner_recommends_enterprise_reference_summit_for_terminal_flagship_heat() -> (
    None
):
    state = make_state(
        make_product("Enterprise Reference Summit Planning Core"),
    ...
    state.finance.board_pressure = 40
    state.finance.governance_risk = 66
    planner = build_finance_planner(
        state.company,
        state.finance,
        state.turn_history,
        ...
        high_value_risk_accounts=2,
        premium_revenue_at_risk_value=Decimal("6400.00"),
        revenue_at_risk_value=Decimal("7200.00"),
        white_glove_queue_risk_accounts=3,
        premium_queue_risk_accounts=3,
        enterprise_queue_risk_accounts=4,
        renewal_queue_risk_accounts=2,
        renewal_pressure_value=Decimal("3800.00"),
        support_lane_focus=SupportLaneFocus.BILLING,
        support_hotspot_lane=SupportLaneFocus.ENTERPRISE,
        support_hotspot_lane_overflow=6,
        hotspot_lane_account_count=4,
    ...
    assert "run_enterprise_reference_summit" in planner.recommended_actions
...
def test_finance_planner_recommends_billing_dispute_cabinet_for_extreme_covenant_heat() -> None:
    state = make_state(
        make_product("Billing Dispute Cabinet Planning Core"),
        cash_on_hand=Decimal("5000.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        support_escalations=3,
        renewal_queue_risk_accounts=2,
        renewal_pressure_value=Decimal("2900.00"),
        support_lane_focus=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane=SupportLaneFocus.BILLING,
        support_hotspot_lane_overflow=2,
        hotspot_lane_account_count=2,
    ...
    assert "run_billing_dispute_cabinet" in planner.recommended_actions
    assert any("billing dispute cabinet" in step for step in planner.action_sequence)


def test_finance_planner_recommends_billing_settlement_board_for_extreme_collections_heat() -> None:
    state = make_state(
        make_product("Billing Settlement Board Planning Core"),
        cash_on_hand=Decimal("4200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-860.00"),
        capital_plan=state.capital_plan,
        support_backlog=12,
        support_escalations=4,
        ...
        dominant_endgame_pressure="independence_discipline",
        commercial_fragility=74,
        capital_fragility=76,
    )

    assert "run_billing_settlement_board" in planner.recommended_actions
    assert any("billing settlement board" in step for step in planner.action_sequence)


def test_finance_planner_recommends_billing_cash_war_room_for_terminal_collections_heat() -> None:
    state = make_state(
        make_product("Billing Cash War Room Planning Core"),
    ...
    state.finance.covenant_risk = 26
    state.finance.board_pressure = 34
    state.finance.debt_principal = Decimal("4200.00")
    planner = build_finance_planner(
        state.company,
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-940.00"),
        capital_plan=state.capital_plan,
        support_backlog=14,
        support_escalations=5,
        renewal_queue_risk_accounts=3,
        renewal_pressure_value=Decimal("4600.00"),
        support_lane_focus=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane=SupportLaneFocus.BILLING,
        support_hotspot_lane_overflow=4,
        hotspot_lane_account_count=3,
        ...
        capital_fragility=80,
    )

    assert "run_billing_cash_war_room" in planner.recommended_actions
...
def test_finance_planner_recommends_billing_liquidity_command_for_terminal_covenant_heat() -> None:
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-1020.00"),
        capital_plan=state.capital_plan,
        support_backlog=16,
        support_escalations=6,
        renewal_queue_risk_accounts=3,
        renewal_pressure_value=Decimal("5000.00"),
        support_lane_focus=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane=SupportLaneFocus.BILLING,
        support_hotspot_lane_overflow=5,
        hotspot_lane_account_count=3,
        ...
        dominant_endgame_pressure="independence_discipline",
        commercial_fragility=80,
        capital_fragility=84,
    )

    assert "run_billing_liquidity_command" in planner.recommended_actions
    assert any("billing liquidity command" in step for step in planner.action_sequence)


def test_finance_planner_recommends_billing_liquidity_summit_for_terminal_covenant_heat() -> None:
    state = make_state(
        make_product("Billing Liquidity Summit Planning Core"),
        cash_on_hand=Decimal("3400.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    state.finance.covenant_risk = 32
    state.finance.board_pressure = 40
    state.finance.debt_principal = Decimal("5200.00")
    planner = build_finance_planner(
        state.company,
        state.finance,
        state.turn_history,
        ...
        renewal_pressure_value=Decimal("5600.00"),
        support_lane_focus=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane=SupportLaneFocus.BILLING,
        support_hotspot_lane_overflow=6,
        hotspot_lane_account_count=4,
        ...
        dominant_endgame_pressure="independence_discipline",
        commercial_fragility=84,
        capital_fragility=88,
    )

    assert "run_billing_liquidity_summit" in planner.recommended_actions
...
def test_finance_planner_recommends_onboarding_launch_cell_for_lane_overflow() -> None:
    state = make_state(
        make_product("Onboarding Launch Cell Planning Core"),
        cash_on_hand=Decimal("5200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    planner = build_finance_planner(
        ...
        support_backlog=15,
        support_escalations=4,
        revenue_at_risk_value=Decimal("2300.00"),
        support_lane_focus=SupportLaneFocus.BALANCED,
        support_hotspot_lane=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane_overflow=4,
        hotspot_lane_account_count=3,
    ...
    assert "run_onboarding_launch_cell" in planner.recommended_actions
    assert any("onboarding launch cell" in step for step in planner.action_sequence)


def test_finance_planner_recommends_onboarding_retention_mesh_for_extreme_lane_overflow() -> None:
    state = make_state(
        make_product("Onboarding Retention Mesh Planning Core"),
        cash_on_hand=Decimal("5200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    assert "run_onboarding_retention_mesh" in planner.recommended_actions
    assert any("onboarding retention mesh" in step for step in planner.action_sequence)


def test_finance_planner_recommends_onboarding_assurance_grid_for_impl_drag() -> None:
    state = make_state(
        make_product("Onboarding Assurance Grid Planning Core"),
        cash_on_hand=Decimal("5000.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    planner = build_finance_planner(
        ...
        support_backlog=23,
        support_escalations=6,
        revenue_at_risk_value=Decimal("3900.00"),
        renewal_pressure_value=Decimal("3200.00"),
        support_lane_focus=SupportLaneFocus.BALANCED,
        support_hotspot_lane=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane_overflow=5,
    ...
    assert "run_onboarding_assurance_grid" in planner.recommended_actions
    assert any("onboarding assurance grid" in step for step in planner.action_sequence)


def test_finance_planner_recommends_onboarding_durability_mesh_for_terminal_impl_drag() -> None:
    state = make_state(
        make_product("Onboarding Durability Mesh Planning Core"),
        cash_on_hand=Decimal("4800.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    planner = build_finance_planner(
        ...
        support_backlog=26,
        support_escalations=7,
        revenue_at_risk_value=Decimal("4400.00"),
        renewal_pressure_value=Decimal("3400.00"),
        support_lane_focus=SupportLaneFocus.BALANCED,
        support_hotspot_lane=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane_overflow=6,
    ...
    assert "run_onboarding_durability_mesh" in planner.recommended_actions
    assert any("onboarding durability mesh" in step for step in planner.action_sequence)


def test_finance_planner_recommends_onboarding_continuity_lattice_for_terminal_impl_drag() -> None:
    state = make_state(
        make_product("Onboarding Continuity Lattice Planning Core"),
        cash_on_hand=Decimal("4500.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.BALANCED,
            source_preference=CapitalSourcePreference.ANGEL,
            reserve_target=Decimal("6200.00"),
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-920.00"),
        capital_plan=state.capital_plan,
        support_backlog=28,
...
def test_finance_planner_recommends_capital_reallocation_grid_for_reset_heat() -> None:
    state = make_state(
        make_product("Capital Reallocation Grid Planning Core"),
        cash_on_hand=Decimal("5000.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        focus_alignment_gap=3,
        channel_conflict_index=32,
        paused_dependency_score=48,
        hotspot_dependency_score=82,
        hotspot_revenue_share_percent=42,
        volatile_revenue_share_percent=28,
        hotspot_channel="reseller",
        strategic_outlook="board_reset",
        dominant_endgame_pressure="board_reset_risk",
        commercial_fragility=74,
    ...
    assert "set_capital_reallocation_grid" in planner.recommended_actions
    assert any("capital reallocation grid" in step for step in planner.action_sequence)
...
def test_finance_planner_recommends_terminal_liquidity_controls_for_extreme_path_heat() -> None:
    state = make_state(
        make_product("Terminal Liquidity Controls Planning Core"),
    ...
    planner = build_finance_planner(
        ...
        dominant_endgame_pressure="board_reset_risk",
        commercial_fragility=74,
        capital_fragility=78,
    )

    assert "set_terminal_liquidity_controls" in planner.recommended_actions
    assert any("terminal liquidity controls" in step for step in planner.action_sequence)
...
def test_finance_planner_recommends_path_resilience_grid_for_extreme_path_heat() -> None:
    state = make_state(
        make_product("Path Resilience Grid Planning Core"),
        cash_on_hand=Decimal("4000.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    assert "set_path_resilience_grid" in planner.recommended_actions
    assert any("path resilience grid" in step for step in planner.action_sequence)


def test_finance_planner_recommends_balance_sheet_recovery_mesh_for_terminal_path_heat() -> None:
    state = make_state(
        make_product("Balance Sheet Recovery Mesh Planning Core"),
        cash_on_hand=Decimal("3800.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    state.finance.board_pressure = 40
    state.finance.governance_risk = 60
    state.finance.covenant_risk = 24
    planner = build_finance_planner(
        state.company,
        state.finance,
    ...
    assert "set_balance_sheet_recovery_mesh" in planner.recommended_actions
    assert any("balance-sheet recovery mesh" in step for step in planner.action_sequence)


def test_finance_planner_recommends_terminal_recovery_lattice_for_extreme_path_heat() -> None:
    state = make_state(
        make_product("Terminal Recovery Lattice Planning Core"),
        cash_on_hand=Decimal("3600.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-1040.00"),
        capital_plan=state.capital_plan,
        support_backlog=20,
        support_escalations=7,
        enterprise_queue_risk_accounts=3,
    ...
    assert "set_terminal_recovery_lattice" in planner.recommended_actions
    assert any("terminal recovery lattice" in step for step in planner.action_sequence)


def test_finance_planner_recommends_terminal_continuity_matrix_for_extreme_path_heat() -> None:
    state = make_state(
        make_product("Terminal Continuity Matrix Planning Core"),
        cash_on_hand=Decimal("3400.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-1140.00"),
        capital_plan=state.capital_plan,
        support_backlog=22,
        support_escalations=8,
        enterprise_queue_risk_accounts=3,
        ...
        hotspot_lane_account_count=5,
        focus_alignment_gap=4,
        hotspot_dependency_score=102,
        strategic_outlook="board_reset",
        dominant_endgame_pressure="board_reset_risk",
        commercial_fragility=88,
        capital_fragility=90,
    )
...
def test_finance_planner_recommends_channel_conflict_lattice_for_hotspot_dependency() -> None:
    state = make_state(
        make_product("Channel Conflict Lattice Planning Core"),
        cash_on_hand=Decimal("4700.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        hotspot_lane_account_count=3,
        focus_alignment_gap=2,
        channel_conflict_index=38,
        paused_dependency_score=60,
        hotspot_dependency_score=94,
        hotspot_revenue_share_percent=50,
        volatile_revenue_share_percent=28,
        recovery_drag_score=42,
        hotspot_channel="integration",
        strategic_outlook="strategic_acquisition",
        dominant_endgame_pressure="acquirer_diligence",
        commercial_fragility=78,
    ...
    assert "run_channel_conflict_lattice" in planner.recommended_actions
    assert any("conflict lattice" in step for step in planner.action_sequence)


def test_finance_planner_recommends_channel_resilience_grid_for_terminal_dependency_heat() -> None:
    state = make_state(
        make_product("Channel Resilience Grid Planning Core"),
        cash_on_hand=Decimal("4500.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        hotspot_lane_account_count=3,
        focus_alignment_gap=2,
        channel_conflict_index=40,
        paused_dependency_score=64,
        hotspot_dependency_score=98,
        hotspot_revenue_share_percent=52,
        volatile_revenue_share_percent=30,
        recovery_drag_score=48,
        hotspot_channel="integration",
        strategic_outlook="strategic_acquisition",
        dominant_endgame_pressure="acquirer_diligence",
        commercial_fragility=82,
    ...
    assert "run_channel_resilience_grid" in planner.recommended_actions
    assert any("channel resilience grid" in step for step in planner.action_sequence)


def test_finance_planner_recommends_channel_continuity_matrix_for_terminal_dependency_heat() -> (
    None
):
    state = make_state(
        make_product("Channel Continuity Matrix Planning Core"),
        cash_on_hand=Decimal("4200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        hotspot_lane_account_count=4,
        focus_alignment_gap=3,
        channel_conflict_index=44,
        paused_dependency_score=70,
        hotspot_dependency_score=104,
        hotspot_revenue_share_percent=56,
        volatile_revenue_share_percent=34,
        recovery_drag_score=52,
        hotspot_channel="integration",
        strategic_outlook="strategic_acquisition",
        dominant_endgame_pressure="acquirer_diligence",
        commercial_fragility=86,
        capital_fragility=72,
    )
...
def test_ipo_demand_floor_event_can_fund_floor() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="ipo_readiness_launchpad",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=29,
            resolved_turn=29,
            selected_option_id="fund_book_stability_panel",
            selected_option_label="Fund the book-stability panel",
            result_text="Book stability panel funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Demand Floor Flagship",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4000.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "ipo_demand_floor"
    )
    ...
    outcome = resolve_pending_event(state, "fund_demand_floor")
    ...
    assert outcome.history_entry.event_id == "ipo_demand_floor"


def test_ipo_price_support_commitment_event_can_fund_commitment() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="ipo_readiness_launchpad",
    ...
    product = state.products[0]
    state.company.current_turn = 32
    state.company.cash_on_hand = Decimal("5600.00")
    state.company.reputation = 86
    state.finance.board_confidence = 72
    state.finance.board_score = 66
    state.finance.board_pressure = 36
    state.finance.governance_risk = 30
    state.finance.board_warning_level = 2
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=31,
            resolved_turn=31,
            selected_option_id="fund_demand_floor",
            selected_option_label="Fund the demand floor",
            result_text="Demand floor funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Price Support Flagship",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4200.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "ipo_price_support_commitment"
    )
    ...
    outcome = resolve_pending_event(state, "fund_price_support_commitment")
    ...
    assert outcome.history_entry.event_id == "ipo_price_support_commitment"


def test_buyer_synergy_holdback_event_can_fund_holdback() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="acquisition_diligence_sprint",
    ...
    product = state.products[0]
    state.company.current_turn = 31
    state.company.cash_on_hand = Decimal("5600.00")
    state.finance.board_confidence = 48
    state.finance.board_score = 44
    state.finance.board_pressure = 30
    state.finance.governance_risk = 24
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=30,
            resolved_turn=30,
            selected_option_id="fund_close_war_room",
            selected_option_label="Fund the close war room",
            result_text="Close war room funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Synergy Holdback Anchor",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("3400.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    state.partnerships = [
        PartnershipDeal(
            ...
            conflict_pressure=56,
            enablement_level=28,
            sourced_revenue=Decimal("3500.00"),
            rev_share_rate=Decimal("0.2200"),
        )
    ]
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "buyer_synergy_holdback"
    )
    ...
    outcome = resolve_pending_event(state, "fund_synergy_holdback")
    ...
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "buyer_synergy_holdback"


def test_buyer_close_certainty_grid_event_can_fund_grid() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="acquisition_diligence_sprint",
    ...
    product = state.products[0]
    state.company.current_turn = 33
    state.company.cash_on_hand = Decimal("5800.00")
    state.finance.board_confidence = 50
    state.finance.board_score = 46
    state.finance.board_pressure = 32
    state.finance.governance_risk = 26
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=32,
            resolved_turn=32,
            selected_option_id="fund_synergy_holdback",
            selected_option_label="Fund the synergy holdback",
            result_text="Synergy holdback funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Close Certainty Anchor",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("3600.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    state.partnerships = [
        PartnershipDeal(
            name="Close Certainty Integration",
            product_id=product.id,
            channel=PartnerChannel.INTEGRATION,
            status=PartnershipStatus.RECOVERY,
            quality=62,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "buyer_close_certainty_grid"
    )
    ...
    outcome = resolve_pending_event(state, "fund_close_certainty_grid")
    ...
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "buyer_close_certainty_grid"


def test_independence_cash_conversion_pact_event_can_ratify_pact() -> None:
    product = make_product("Cash Conversion Pact Core", lifecycle_stage=LifecycleStage.MATURE)
    state = make_state(
        product,
        cash_on_hand=Decimal("3000.00"),
        current_turn=28,
        finance=FinanceState(
            debt_principal=Decimal("3800.00"),
            loan_interest_rate=Decimal("0.0310"),
            investor_pressure=24,
            covenant_risk=24,
            board_confidence=58,
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=27,
                resolved_turn=27,
                selected_option_id="ratify_treasury_lattice",
                selected_option_label="Ratify the treasury lattice",
                result_text="Treasury lattice ratified.",
            )
        ],
    )
    state.capital_plan.reserve_target = Decimal("5600.00")
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "independence_cash_conversion_pact"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_cash_conversion_pact")
    ...
    assert outcome.state.finance.investor_pressure < state.finance.investor_pressure
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.history_entry.event_id == "independence_cash_conversion_pact"


def test_independence_cash_discipline_statute_event_can_ratify_statute() -> None:
    product = make_product("Cash Discipline Statute Core", lifecycle_stage=LifecycleStage.MATURE)
    state = make_state(
        product,
        cash_on_hand=Decimal("2800.00"),
        current_turn=30,
        finance=FinanceState(
            debt_principal=Decimal("4000.00"),
            loan_interest_rate=Decimal("0.0310"),
            investor_pressure=26,
            covenant_risk=24,
            board_confidence=56,
            board_pressure=30,
        ),
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=29,
                resolved_turn=29,
                selected_option_id="ratify_cash_conversion_pact",
                selected_option_label="Ratify the cash-conversion pact",
                result_text="Cash-conversion pact ratified.",
            )
        ],
    )
    state.capital_plan.reserve_target = Decimal("6000.00")
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "independence_cash_discipline_statute"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_cash_discipline_statute")
    ...
    assert outcome.state.finance.investor_pressure < state.finance.investor_pressure
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.history_entry.event_id == "independence_cash_discipline_statute"


def test_reseller_margin_reconciliation_event_can_fund_reconciliation() -> None:
    product = make_product("Margin Reconciliation Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=22,
                resolved_turn=22,
                selected_option_id="fund_reference_scorecard",
                selected_option_label="Fund the reference scorecard",
                result_text="Reference scorecard funded.",
            )
        ],
        current_turn=24,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "reseller_margin_reconciliation"
    )
    ...
    outcome = resolve_pending_event(state, "fund_margin_reconciliation")
    ...
    assert outcome.history_entry.event_id == "reseller_margin_reconciliation"


def test_reseller_service_dividend_event_can_fund_dividend() -> None:
    product = make_product("Service Dividend Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=24,
                resolved_turn=24,
                selected_option_id="fund_margin_reconciliation",
                selected_option_label="Fund the margin reconciliation",
                result_text="Margin reconciliation funded.",
            )
        ],
        current_turn=26,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "reseller_service_dividend"
    )
    ...
    outcome = resolve_pending_event(state, "fund_service_dividend")
    ...
    assert outcome.history_entry.event_id == "reseller_service_dividend"


def test_integration_reliability_bridge_event_can_fund_bridge() -> None:
    product = make_product("Reliability Bridge Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Reliability Bridge Account",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("2900.00"),
        support_tier=SupportTier.PRIORITY,
        satisfaction=56,
        onboarding_health=40,
        support_load=32,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=23,
                resolved_turn=23,
                selected_option_id="fund_hypercare_sla",
                selected_option_label="Fund the hypercare SLA",
                result_text="Hypercare SLA funded.",
            )
        ],
        current_turn=25,
        cash_on_hand=Decimal("7300.00"),
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "integration_reliability_bridge"
    )
    ...
    outcome = resolve_pending_event(state, "fund_reliability_bridge")
    ...
    assert outcome.state.customer_accounts[0].support_load < state.customer_accounts[0].support_load
    assert outcome.history_entry.event_id == "integration_reliability_bridge"


def test_integration_reliability_warranty_event_can_fund_warranty() -> None:
    product = make_product("Reliability Warranty Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=25,
                resolved_turn=25,
                selected_option_id="fund_reliability_bridge",
                selected_option_label="Fund the reliability bridge",
                result_text="Reliability bridge funded.",
            )
        ],
        current_turn=27,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "integration_reliability_warranty"
    )
    ...
    outcome = resolve_pending_event(state, "fund_reliability_warranty")
    ...
    assert outcome.state.customer_accounts[0].support_load < state.customer_accounts[0].support_load
    assert outcome.history_entry.event_id == "integration_reliability_warranty"


def test_marketplace_dispute_escrow_event_can_fund_escrow() -> None:
    product = make_product("Dispute Escrow Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Dispute Escrow Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2200.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=24,
                resolved_turn=24,
                selected_option_id="fund_refund_arbitration",
                selected_option_label="Fund the refund arbitration",
                result_text="Refund arbitration funded.",
            )
        ],
        current_turn=26,
        cash_on_hand=Decimal("7300.00"),
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "marketplace_dispute_escrow"
    )
    ...
    outcome = resolve_pending_event(state, "fund_dispute_escrow")
    ...
    assert outcome.history_entry.event_id == "marketplace_dispute_escrow"


def test_marketplace_refund_surety_event_can_fund_surety() -> None:
    product = make_product("Refund Surety Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Refund Surety Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=26,
                resolved_turn=26,
                selected_option_id="fund_dispute_escrow",
                selected_option_label="Fund the dispute escrow",
                result_text="Dispute escrow funded.",
            )
        ],
        current_turn=28,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "marketplace_refund_surety"
    )
    ...
    outcome = resolve_pending_event(state, "fund_refund_surety")
    ...
    assert outcome.history_entry.event_id == "marketplace_refund_surety"


def test_board_reset_resilience_statute_event_can_ratify_statute() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="board_recovery_crucible",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=31,
            resolved_turn=31,
            selected_option_id="ratify_execution_council",
            selected_option_label="Ratify the execution council",
            result_text="Execution council ratified.",
        )
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "board_reset_resilience_statute"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_resilience_statute")
    ...
    assert outcome.state.finance.board_score > state.finance.board_score
    assert outcome.state.finance.board_resolution_due is False
    assert outcome.history_entry.event_id == "board_reset_resilience_statute"


def test_board_reset_recovery_ordinance_event_can_ratify_ordinance() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="board_recovery_crucible",
    )
    state.company.current_turn = 34
    state.company.cash_on_hand = Decimal("5600.00")
    state.finance.board_pressure = 40
    state.finance.governance_risk = 50
    state.finance.board_confidence = 30
    state.finance.board_score = 26
    state.finance.board_portfolio_focus_score = 32
    state.finance.board_warning_level = 2
    state.finance.restructuring_pressure = 28
    state.finance.board_resolution_due = True
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=33,
            resolved_turn=33,
            selected_option_id="ratify_resilience_statute",
            selected_option_label="Ratify the resilience statute",
            result_text="Resilience statute ratified.",
        )
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "board_reset_recovery_ordinance"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_recovery_ordinance")
    ...
    assert outcome.state.finance.board_score > state.finance.board_score
    assert outcome.state.finance.board_resolution_due is False
    assert outcome.history_entry.event_id == "board_reset_recovery_ordinance"


def test_ipo_price_band_committee_event_can_fund_committee() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="ipo_readiness_launchpad",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=35,
            resolved_turn=35,
            selected_option_id="fund_price_support_commitment",
            selected_option_label="Fund the price-support commitment",
            result_text="Price-support commitment funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Price Band Flagship",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4600.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "ipo_price_band_committee"
    )
    ...
    outcome = resolve_pending_event(state, "fund_price_band_committee")
    ...
    assert outcome.history_entry.event_id == "ipo_price_band_committee"


def test_ipo_book_support_panel_event_can_fund_panel() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="ipo_readiness_launchpad",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=36,
            resolved_turn=36,
            selected_option_id="fund_price_band_committee",
            selected_option_label="Fund the price-band committee",
            result_text="Price-band committee funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Book Support Flagship",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4800.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "ipo_book_support_panel"
    )
    ...
    outcome = resolve_pending_event(state, "fund_book_support_panel")
...
def test_buyer_close_warranty_event_can_fund_warranty() -> None:
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=34,
            resolved_turn=34,
            selected_option_id="fund_close_certainty_grid",
            selected_option_label="Fund the close-certainty grid",
            result_text="Close-certainty grid funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Close Warranty Anchor",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4000.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    state.partnerships = [
        PartnershipDeal(
            name="Close Warranty Integration",
            product_id=product.id,
            channel=PartnerChannel.INTEGRATION,
            status=PartnershipStatus.STRAINED,
            quality=60,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "buyer_close_warranty"
    )
    ...
    outcome = resolve_pending_event(state, "fund_close_warranty")
    ...
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "buyer_close_warranty"


def test_buyer_close_assurance_panel_event_can_fund_panel() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="acquisition_diligence_sprint",
    ...
    state.finance.board_confidence = 54
    state.finance.board_score = 50
    state.finance.board_pressure = 36
    state.finance.governance_risk = 30
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=35,
            resolved_turn=35,
            selected_option_id="fund_close_warranty",
            selected_option_label="Fund the close warranty",
            result_text="Close warranty funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Close Assurance Anchor",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4200.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    state.partnerships = [
        PartnershipDeal(
            name="Close Assurance Integration",
            product_id=product.id,
            channel=PartnerChannel.INTEGRATION,
            status=PartnershipStatus.STRAINED,
            quality=58,
            ...
            enablement_level=18,
            sourced_revenue=Decimal("4600.00"),
            rev_share_rate=Decimal("0.2400"),
        )
    ]
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "buyer_close_assurance_panel"
    )
    ...
    outcome = resolve_pending_event(state, "fund_close_assurance_panel")
...
def test_independence_cash_conversion_charter_event_can_ratify_charter() -> None:
    ...
    state = make_state(
        ...
        current_turn=32,
        finance=FinanceState(
            debt_principal=Decimal("4200.00"),
            loan_interest_rate=Decimal("0.0310"),
            investor_pressure=28,
            covenant_risk=26,
            board_confidence=54,
            board_pressure=32,
        ),
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=31,
                resolved_turn=31,
                selected_option_id="ratify_cash_discipline_statute",
                selected_option_label="Ratify the cash-discipline statute",
                result_text="Cash-discipline statute ratified.",
            )
        ],
    )
    state.capital_plan.reserve_target = Decimal("6200.00")
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "independence_cash_conversion_charter"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_cash_conversion_charter")
    ...
    assert outcome.state.finance.investor_pressure < state.finance.investor_pressure
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.history_entry.event_id == "independence_cash_conversion_charter"


def test_independence_cash_surety_charter_event_can_ratify_charter() -> None:
    product = make_product("Cash Surety Charter Core", lifecycle_stage=LifecycleStage.MATURE)
    state = make_state(
        product,
        cash_on_hand=Decimal("2500.00"),
        current_turn=33,
        finance=FinanceState(
            debt_principal=Decimal("4400.00"),
            loan_interest_rate=Decimal("0.0310"),
            investor_pressure=30,
            covenant_risk=28,
            board_confidence=52,
            board_pressure=34,
        ),
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=32,
                resolved_turn=32,
                selected_option_id="ratify_cash_conversion_charter",
                selected_option_label="Ratify the cash-conversion charter",
                result_text="Cash-conversion charter ratified.",
            )
        ],
    )
    state.capital_plan.reserve_target = Decimal("6600.00")
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "independence_cash_surety_charter"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_cash_surety_charter")
...
def test_reseller_service_escrow_event_can_fund_escrow() -> None:
    product = make_product("Service Escrow Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Escrow Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=26,
                resolved_turn=26,
                selected_option_id="fund_service_dividend",
                selected_option_label="Fund the service dividend",
                result_text="Service dividend funded.",
            )
        ],
        current_turn=28,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "reseller_service_escrow"
    )
    ...
    outcome = resolve_pending_event(state, "fund_service_escrow")
    ...
    assert outcome.history_entry.event_id == "reseller_service_escrow"


def test_reseller_service_surety_event_can_fund_surety() -> None:
    product = make_product("Service Surety Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Surety Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
    ...
    partnership = PartnershipDeal(
        name="Surety Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.STRAINED,
        quality=56,
        risk=64,
        conflict_pressure=60,
        enablement_level=20,
        sourced_revenue=Decimal("3600.00"),
        sourced_users=44,
        rev_share_rate=Decimal("0.2300"),
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=27,
                resolved_turn=27,
                selected_option_id="fund_service_escrow",
                selected_option_label="Fund the service escrow",
                result_text="Service escrow funded.",
            )
        ],
        current_turn=29,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "reseller_service_surety"
    )
    ...
    outcome = resolve_pending_event(state, "fund_service_surety")
...
def test_integration_cutover_warranty_event_can_fund_warranty() -> None:
    ...
    partnership = PartnershipDeal(
        name="Cutover Warranty Integration",
        product_id=product.id,
        channel=PartnerChannel.INTEGRATION,
        status=PartnershipStatus.RECOVERY,
        quality=56,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=27,
                resolved_turn=27,
                selected_option_id="fund_reliability_warranty",
                selected_option_label="Fund the reliability warranty",
                result_text="Reliability warranty funded.",
            )
        ],
        current_turn=29,
        cash_on_hand=Decimal("8000.00"),
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "integration_cutover_warranty"
    )
    ...
    outcome = resolve_pending_event(state, "fund_cutover_warranty")
    ...
    assert outcome.state.customer_accounts[0].support_load < state.customer_accounts[0].support_load
    assert outcome.history_entry.event_id == "integration_cutover_warranty"


def test_integration_cutover_surety_event_can_fund_surety() -> None:
    product = make_product("Cutover Surety Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Cutover Surety Account",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3400.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Cutover Surety Integration",
        product_id=product.id,
        channel=PartnerChannel.INTEGRATION,
        status=PartnershipStatus.RECOVERY,
        quality=54,
        ...
        conflict_pressure=62,
        enablement_level=20,
        sourced_revenue=Decimal("4000.00"),
        sourced_users=46,
        rev_share_rate=Decimal("0.2300"),
    )
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=28,
                resolved_turn=28,
                selected_option_id="fund_cutover_warranty",
                selected_option_label="Fund the cutover warranty",
                result_text="Cutover warranty funded.",
            )
        ],
        current_turn=30,
        cash_on_hand=Decimal("8400.00"),
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "integration_cutover_surety"
    )
    ...
    outcome = resolve_pending_event(state, "fund_cutover_surety")
...
def test_marketplace_refund_backstop_event_can_fund_backstop() -> None:
    product = make_product("Refund Backstop Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Refund Backstop Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2400.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=28,
                resolved_turn=28,
                selected_option_id="fund_refund_surety",
                selected_option_label="Fund the refund surety",
                result_text="Refund surety funded.",
            )
        ],
        current_turn=30,
        cash_on_hand=Decimal("8200.00"),
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "marketplace_refund_backstop"
    )
    ...
    outcome = resolve_pending_event(state, "fund_refund_backstop")
    ...
    assert outcome.history_entry.event_id == "marketplace_refund_backstop"


def test_marketplace_refund_lattice_event_can_fund_lattice() -> None:
    product = make_product("Refund Lattice Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Refund Lattice Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2500.00"),
        support_tier=SupportTier.PRIORITY,
        ...
        churn_risk=30,
        invoice_risk=42,
        failed_payment_risk=38,
        status=CustomerAccountStatus.AT_RISK,
    )
    partnership = PartnershipDeal(
        name="Lattice Marketplace",
        product_id=product.id,
        channel=PartnerChannel.MARKETPLACE,
        status=PartnershipStatus.STRAINED,
        quality=56,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=29,
                resolved_turn=29,
                selected_option_id="fund_refund_backstop",
                selected_option_label="Fund the refund backstop",
                result_text="Refund backstop funded.",
            )
        ],
        current_turn=31,
        cash_on_hand=Decimal("8600.00"),
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "marketplace_refund_lattice"
    )
    ...
    outcome = resolve_pending_event(state, "fund_refund_lattice")
...
def test_board_reset_execution_lattice_event_can_ratify_lattice() -> None:
    ...
    state.finance.board_confidence = 28
    state.finance.board_score = 24
    state.finance.board_portfolio_focus_score = 30
    state.finance.board_warning_level = 2
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=35,
            resolved_turn=35,
            selected_option_id="ratify_recovery_ordinance",
            selected_option_label="Ratify the recovery ordinance",
            result_text="Recovery ordinance ratified.",
        )
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "board_reset_execution_lattice"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_execution_lattice")
    ...
    assert outcome.state.finance.board_score > state.finance.board_score
    assert outcome.state.finance.board_resolution_due is False
    assert outcome.history_entry.event_id == "board_reset_execution_lattice"


def test_board_reset_continuity_covenant_event_can_ratify_covenant() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="board_recovery_crucible",
    ...
    state.finance.board_confidence = 26
    state.finance.board_score = 22
    state.finance.board_portfolio_focus_score = 28
    state.finance.board_warning_level = 2
    state.finance.restructuring_pressure = 34
    state.finance.board_resolution_due = True
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=36,
            resolved_turn=36,
            selected_option_id="ratify_execution_lattice",
            selected_option_label="Ratify the execution lattice",
            result_text="Execution lattice ratified.",
        )
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "board_reset_continuity_covenant"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_continuity_covenant")
...
def test_ipo_pricing_guardrail_event_can_fund_guardrail() -> None:
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=27,
            resolved_turn=27,
            selected_option_id="fund_allocation_lock",
            selected_option_label="Fund the allocation lock",
            result_text="Allocation lock funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Pricing Guardrail Anchor",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("3800.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "ipo_pricing_guardrail"
    )
    ...
    assert outcome.state.finance.governance_risk < state.finance.governance_risk
    assert outcome.state.customer_accounts[0].support_load < state.customer_accounts[0].support_load
    assert outcome.history_entry.event_id == "ipo_pricing_guardrail"


def test_buyer_close_committee_event_can_staff_committee() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="acquisition_diligence_sprint",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=25,
            resolved_turn=25,
            selected_option_id="staff_close_cadence",
            selected_option_label="Staff the close cadence",
            result_text="Close cadence staffed.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Close Committee Anchor",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("3200.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    state.partnerships = [
        PartnershipDeal(
            name="Close Committee Integration",
            product_id=product.id,
            channel=PartnerChannel.INTEGRATION,
            status=PartnershipStatus.RECOVERY,
            quality=62,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "buyer_close_committee"
    )
    ...
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "buyer_close_committee"


def test_independence_liquidity_grid_event_can_ratify_grid() -> None:
    product = make_product("Liquidity Grid Core", lifecycle_stage=LifecycleStage.MATURE)
    state = make_state(
        product,
        cash_on_hand=Decimal("3300.00"),
        current_turn=20,
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=19,
                resolved_turn=19,
                selected_option_id="ratify_margin_charter",
                selected_option_label="Ratify the margin charter",
                result_text="Margin charter ratified.",
            )
        ],
    )
    state.capital_plan.reserve_target = Decimal("5400.00")
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "independence_liquidity_grid"
    )
    ...
    assert outcome.state.finance.investor_pressure < state.finance.investor_pressure
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.history_entry.event_id == "independence_liquidity_grid"


def test_reseller_service_council_event_can_fund_council() -> None:
    product = make_product("Service Council Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Service Council Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("1800.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=20,
                resolved_turn=20,
                selected_option_id="fund_recovery_compact",
                selected_option_label="Fund the recovery compact",
                result_text="Recovery compact funded.",
            )
        ],
        current_turn=22,
        cash_on_hand=Decimal("6900.00"),
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "reseller_service_council"
    )
    ...
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.state.partnerships[0].status is PartnershipStatus.RECOVERY
    assert outcome.history_entry.event_id == "reseller_service_council"


def test_integration_hypercare_grid_event_can_fund_grid() -> None:
    product = make_product("Hypercare Grid Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Hypercare Grid Account",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("2800.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=21,
                resolved_turn=21,
                selected_option_id="fund_cutover_command",
                selected_option_label="Fund the cutover command",
                result_text="Cutover command funded.",
            )
        ],
        current_turn=23,
        cash_on_hand=Decimal("7000.00"),
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "integration_hypercare_grid"
    )
    ...
    assert outcome.state.customer_accounts[0].support_load < state.customer_accounts[0].support_load
    assert outcome.history_entry.event_id == "integration_hypercare_grid"


def test_marketplace_refund_bench_event_can_fund_bench() -> None:
    product = make_product("Refund Bench Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Refund Bench Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2200.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=22,
                resolved_turn=22,
                selected_option_id="fund_penalty_panel",
                selected_option_label="Fund the penalty panel",
                result_text="Penalty panel funded.",
            )
        ],
        current_turn=24,
        cash_on_hand=Decimal("7000.00"),
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "marketplace_refund_bench"
    )
    ...
    assert outcome.history_entry.event_id == "marketplace_refund_bench"


def test_board_reset_runway_table_event_can_ratify_table() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="board_recovery_crucible",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=27,
            resolved_turn=27,
            selected_option_id="ratify_cash_charter",
            selected_option_label="Ratify the cash charter",
            result_text="Cash charter ratified.",
        )
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "board_reset_runway_table"
    )
    ...
    assert outcome.state.finance.board_score > state.finance.board_score
    assert outcome.state.finance.board_resolution_due is False
    assert outcome.history_entry.event_id == "board_reset_runway_table"
...
def test_three_hundred_twenty_turn_board_recovery_crucible_progression_is_seed_stable() -> None:
    def run_once(seed: int) -> tuple[Decimal, int, int, bool, bool, int, int, str | None]:
        state = create_new_game(
            DEFAULT_COMPANY_NAME,
            DEFAULT_PRODUCT_NAME,
    ...
    assert run_once(3201) == run_once(3201)


def test_three_hundred_forty_turn_board_recovery_crucible_progression_is_seed_stable() -> None:
    def run_once(seed: int) -> tuple[Decimal, int, int, bool, bool, int, int, str | None]:
        state = create_new_game(
            DEFAULT_COMPANY_NAME,
            DEFAULT_PRODUCT_NAME,
    ...
    assert run_once(3401) == run_once(3401)


def test_three_hundred_sixty_turn_board_recovery_crucible_progression_is_seed_stable() -> None:
    def run_once(seed: int) -> tuple[Decimal, int, int, bool, bool, int, int, str | None]:
        state = create_new_game(
            DEFAULT_COMPANY_NAME,
            DEFAULT_PRODUCT_NAME,
    ...
    assert run_once(3601) == run_once(3601)


def test_three_hundred_eighty_turn_board_recovery_crucible_progression_is_seed_stable() -> None:
    def run_once(seed: int) -> tuple[Decimal, int, int, bool, bool, int, int, str | None]:
        state = create_new_game(
            DEFAULT_COMPANY_NAME,
            DEFAULT_PRODUCT_NAME,
    ...
    assert run_once(3801) == run_once(3801)


def test_four_hundred_turn_board_recovery_crucible_progression_is_seed_stable() -> None:
    def run_once(seed: int) -> tuple[Decimal, int, int, bool, bool, int, int, str | None]:
        state = create_new_game(
            DEFAULT_COMPANY_NAME,
            DEFAULT_PRODUCT_NAME,
    ...
    assert run_once(4001) == run_once(4001)


def test_four_hundred_twenty_turn_board_recovery_crucible_progression_is_seed_stable() -> None:
    def run_once(seed: int) -> tuple[Decimal, int, int, bool, bool, int, int, str | None]:
        state = create_new_game(
            DEFAULT_COMPANY_NAME,
            DEFAULT_PRODUCT_NAME,
...
def test_run_channel_dependency_reset_cools_hotspot_dependency_lane() -> None:
    product = make_product("Channel Dependency Reset Core", target_segment=MarketSegment.ENTERPRISE)
    account = CustomerAccount(
        name="Integration Dependency Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("2800.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Integration Dependency Hotspot",
        product_id=product.id,
        channel=PartnerChannel.INTEGRATION,
        status=PartnershipStatus.STRAINED,
        quality=58,
...
def test_run_channel_confidence_firewall_cools_hotspot_dependency_and_conflict() -> None:
    product = make_product(
        "Channel Confidence Firewall Core", target_segment=MarketSegment.ENTERPRISE
    )
    account = CustomerAccount(
        name="Confidence Firewall Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3000.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Integration Confidence Hotspot",
        product_id=product.id,
        channel=PartnerChannel.INTEGRATION,
        status=PartnershipStatus.STRAINED,
        quality=56,
...
def test_run_channel_durability_mesh_cools_hotspot_dependency_and_conflict() -> None:
    product = make_product("Channel Durability Mesh Core", target_segment=MarketSegment.ENTERPRISE)
    account = CustomerAccount(
        name="Durability Mesh Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
    ...
    partnership = PartnershipDeal(
        name="Integration Durability Hotspot",
        product_id=product.id,
        channel=PartnerChannel.INTEGRATION,
        status=PartnershipStatus.STRAINED,
        quality=54,
...
def test_run_channel_conflict_lattice_cools_terminal_hotspot_dependency_and_conflict() -> None:
    product = make_product("Channel Conflict Lattice Core", target_segment=MarketSegment.ENTERPRISE)
    account = CustomerAccount(
        name="Conflict Lattice Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3600.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Conflict Lattice Integration",
        product_id=product.id,
        channel=PartnerChannel.INTEGRATION,
        status=PartnershipStatus.STRAINED,
        quality=52,
...
def test_run_channel_resilience_grid_cools_terminal_hotspot_dependency_and_conflict() -> None:
    product = make_product("Channel Resilience Grid Core", target_segment=MarketSegment.ENTERPRISE)
    account = CustomerAccount(
        name="Resilience Grid Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("4200.00"),
        support_tier=SupportTier.WHITE_GLOVE,
    ...
    partnership = PartnershipDeal(
        ...
        enablement_level=14,
        sourced_revenue=Decimal("6200.00"),
        sourced_users=72,
...
def test_run_channel_continuity_matrix_cools_terminal_hotspot_dependency_and_conflict() -> None:
    product = make_product(
        "Channel Continuity Matrix Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Continuity Matrix Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("4600.00"),
        support_tier=SupportTier.WHITE_GLOVE,
    ...
    partnership = PartnershipDeal(
        name="Continuity Matrix Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.STRAINED,
        quality=46,
        ...
        enablement_level=12,
        sourced_revenue=Decimal("6800.00"),
        sourced_users=78,
        rev_share_rate=Decimal("0.2700"),
    )
    state = make_state(
        product,
        customer_accounts=[account],
        partnerships=[partnership],
        cash_on_hand=Decimal("35600.00"),
        current_turn=22,
    )
    state.finance.board_pressure = 38
    state.finance.investor_pressure = 32
...
def test_set_path_resilience_grid_realigns_plan_for_extreme_path_heat() -> None:
    product = make_product("Path Resilience Grid Core")
    capital_plan = CapitalPlan(
        mode=CapitalPlanMode.EXPAND,
        source_preference=CapitalSourcePreference.DEBT,
        planning_horizon_turns=6,
        reserve_target=Decimal("4100.00"),
        product_investment_share=36,
        go_to_market_share=41,
        reserve_share=23,
    )
    account = CustomerAccount(
        name="Resilience Queue Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3800.00"),
        support_tier=SupportTier.WHITE_GLOVE,
    ...
    partnership = PartnershipDeal(
        name="Resilience Hotspot Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.RECOVERY,
        quality=56,
    ...
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert "path resilience grid" in outcome.message


def test_set_balance_sheet_recovery_mesh_realigns_plan_for_terminal_path_heat() -> None:
    product = make_product("Balance Sheet Recovery Mesh Core")
    capital_plan = CapitalPlan(
        mode=CapitalPlanMode.EXPAND,
        source_preference=CapitalSourcePreference.DEBT,
        planning_horizon_turns=6,
    ...
    account = CustomerAccount(
        name="Recovery Mesh Queue Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("4200.00"),
        support_tier=SupportTier.WHITE_GLOVE,
    ...
    partnership = PartnershipDeal(
        name="Recovery Mesh Hotspot Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.RECOVERY,
        quality=54,
    ...
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert "balance-sheet recovery mesh" in outcome.message
...
def test_set_capital_reallocation_grid_realigns_plan_for_multi_system_heat() -> None:
    product = make_product("Capital Reallocation Grid Core")
    capital_plan = CapitalPlan(
        mode=CapitalPlanMode.EXPAND,
        source_preference=CapitalSourcePreference.DEBT,
        planning_horizon_turns=6,
    ...
    account = CustomerAccount(
        name="Reallocation Queue Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3800.00"),
        support_tier=SupportTier.WHITE_GLOVE,
    ...
    partnership = PartnershipDeal(
        name="Reallocation Hotspot Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.RECOVERY,
        quality=58,
    ...
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert "capital reallocation grid" in outcome.message
...
def test_set_terminal_liquidity_controls_realigns_plan_for_extreme_board_reset_heat() -> None:
    product = make_product("Terminal Liquidity Controls Core")
    capital_plan = CapitalPlan(
        mode=CapitalPlanMode.EXPAND,
        source_preference=CapitalSourcePreference.DEBT,
        planning_horizon_turns=6,
    ...
    partnership = PartnershipDeal(
        name="Terminal Control Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.RECOVERY,
        quality=60,
    ...
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert "terminal liquidity controls" in outcome.message


def test_set_terminal_recovery_lattice_realigns_plan_for_terminal_multi_path_heat() -> None:
    product = make_product("Terminal Recovery Lattice Core")
    capital_plan = CapitalPlan(
        mode=CapitalPlanMode.EXPAND,
        source_preference=CapitalSourcePreference.DEBT,
        planning_horizon_turns=6,
    ...
    partnership = PartnershipDeal(
        name="Recovery Lattice Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.RECOVERY,
        quality=54,
    ...
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert "terminal recovery lattice" in outcome.message


def test_set_terminal_continuity_matrix_realigns_plan_for_terminal_multi_path_heat() -> None:
    product = make_product("Terminal Continuity Matrix Core")
    capital_plan = CapitalPlan(
        mode=CapitalPlanMode.EXPAND,
        source_preference=CapitalSourcePreference.DEBT,
        planning_horizon_turns=6,
    ...
    account = CustomerAccount(
        name="Continuity Matrix Queue Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("4600.00"),
        support_tier=SupportTier.WHITE_GLOVE,
...
def test_run_enterprise_reference_secretariat_rebuilds_terminal_flagship_trust() -> None:
    product = make_product("Reference Secretariat Core", target_segment=MarketSegment.ENTERPRISE)
    account = CustomerAccount(
        name="Reference Secretariat Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("11600.00"),
        support_tier=SupportTier.WHITE_GLOVE,
        satisfaction=28,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("46000.00"))
    state.support_program.backlog_queue = 26
    state.support_program.escalation_queue = 13
    state.finance.board_pressure = 42
    state.finance.board_confidence = 20
    state.finance.board_score = 20
    state.finance.investor_pressure = 30
...
def test_run_billing_liquidity_secretariat_cools_terminal_collections_extreme_heat() -> None:
    product = make_product("Billing Secretariat Core", target_segment=MarketSegment.SMB)
    account = CustomerAccount(
        name="Liquidity Secretariat Anchor",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("5600.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("43000.00"))
    state.support_program.backlog_queue = 22
    state.support_program.escalation_queue = 11
    state.finance.board_pressure = 42
    state.finance.investor_pressure = 36
    state.finance.covenant_risk = 34
    state.finance.board_confidence = 20
...
def test_run_onboarding_continuity_secretariat_stabilizes_terminal_drag() -> None:
    product = make_product(
        "Onboarding Continuity Secretariat Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Continuity Secretariat Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("39200.00"))
    state.support_program.backlog_queue = 23
    state.support_program.escalation_queue = 11
    state.finance.board_pressure = 38
    state.finance.board_confidence = 28
    state.company.reputation = 54
...
def test_run_channel_durability_statute_recovers_terminal_hotspot_channel() -> None:
    product = make_product(
        "Channel Durability Statute Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Durability Statute Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3200.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Durability Statute Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.STRAINED,
        quality=42,
        risk=82,
        conflict_pressure=78,
        enablement_level=8,
        sourced_revenue=Decimal("7200.00"),
        sourced_users=86,
        rev_share_rate=Decimal("0.2850"),
    )
    state = make_state(
        product,
        customer_accounts=[account],
        partnerships=[partnership],
        cash_on_hand=Decimal("44000.00"),
        current_turn=25,
    )
    state.finance.board_pressure = 42
    state.finance.investor_pressure = 36
...
def test_set_terminal_solvency_statute_shifts_to_deepest_terminal_controls() -> None:
    product = make_product("Terminal Solvency Core", target_segment=MarketSegment.ENTERPRISE)
    capital_plan = CapitalPlan(
        mode=CapitalPlanMode.BALANCED,
        source_preference=CapitalSourcePreference.VENTURE,
        planning_horizon_turns=8,
    ...
    account = CustomerAccount(
        name="Terminal Solvency Account",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("5400.00"),
        support_tier=SupportTier.WHITE_GLOVE,
    ...
    partnership = PartnershipDeal(
        name="Terminal Solvency Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.RECOVERY,
        quality=46,
        ...
        conflict_pressure=68,
        enablement_level=14,
        sourced_revenue=Decimal("5800.00"),
        sourced_users=74,
        rev_share_rate=Decimal("0.2700"),
    )
    state = make_state(
    ...
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert "terminal solvency statute" in outcome.message


def test_finance_planner_recommends_enterprise_reference_secretariat_for_flagship_heat() -> None:
    state = make_state(
        make_product("Enterprise Reference Secretariat Planning Core"),
        cash_on_hand=Decimal("4200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        support_backlog=22,
        support_escalations=8,
        premium_revenue_at_risk_value=Decimal("5600.00"),
        enterprise_queue_risk_accounts=2,
        high_value_risk_accounts=2,
        white_glove_queue_risk_accounts=1,
        revenue_at_risk_value=Decimal("5200.00"),
    ...
    assert "run_enterprise_reference_secretariat" in planner.recommended_actions
    assert any("enterprise reference secretariat" in step for step in planner.action_sequence)


def test_finance_planner_recommends_billing_liquidity_secretariat_for_terminal_covenant_heat() -> (
    None
):
    state = make_state(
        make_product("Billing Liquidity Secretariat Planning Core"),
        cash_on_hand=Decimal("3600.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
            reserve_target=Decimal("6800.00"),
            product_investment_share=34,
            go_to_market_share=42,
            reserve_share=24,
        ),
    ...
    state.finance.covenant_risk = 34
    state.finance.board_pressure = 40
    state.finance.debt_principal = Decimal("4800.00")
    planner = build_finance_planner(
        state.company,
        state.finance,
        state.turn_history,
        ...
        support_backlog=24,
        support_escalations=9,
        revenue_at_risk_value=Decimal("4600.00"),
        renewal_queue_risk_accounts=3,
    ...
    assert "run_billing_liquidity_secretariat" in planner.recommended_actions
    assert any("billing liquidity secretariat" in step for step in planner.action_sequence)


def test_finance_planner_recommends_onboarding_continuity_secretariat_for_terminal_drag() -> None:
    state = make_state(
        make_product("Onboarding Continuity Secretariat Planning Core"),
    ...
    planner = build_finance_planner(
        ...
        capital_plan=state.capital_plan,
        support_backlog=30,
        support_escalations=10,
        revenue_at_risk_value=Decimal("5400.00"),
        renewal_pressure_value=Decimal("4400.00"),
        support_lane_focus=SupportLaneFocus.BALANCED,
        support_hotspot_lane=SupportLaneFocus.ONBOARDING,
    ...
    assert "run_onboarding_continuity_secretariat" in planner.recommended_actions
    assert any("onboarding continuity secretariat" in step for step in planner.action_sequence)


def test_finance_planner_recommends_channel_durability_statute_for_terminal_dependency_heat() -> (
    None
):
    state = make_state(
        make_product("Channel Durability Statute Planning Core"),
        cash_on_hand=Decimal("3900.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        focus_alignment_gap=3,
        channel_conflict_index=46,
        paused_dependency_score=76,
        hotspot_dependency_score=112,
        hotspot_revenue_share_percent=58,
        volatile_revenue_share_percent=38,
        recovery_drag_score=60,
        hotspot_channel="integration",
        strategic_outlook="strategic_acquisition",
        dominant_endgame_pressure="acquirer_diligence",
        commercial_fragility=90,
    ...
    assert "run_channel_durability_statute" in planner.recommended_actions
    assert any("channel durability statute" in step for step in planner.action_sequence)


def test_finance_planner_recommends_terminal_solvency_statute_for_board_reset_heat() -> None:
    state = make_state(
        make_product("Terminal Solvency Statute Planning Core"),
        cash_on_hand=Decimal("3400.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-1260.00"),
        capital_plan=state.capital_plan,
        support_backlog=24,
        support_escalations=9,
        support_lane_focus=SupportLaneFocus.BILLING,
    ...
    assert "set_terminal_solvency_statute" in planner.recommended_actions
    assert any("terminal solvency statute" in step for step in planner.action_sequence)


def test_ipo_book_anchor_statute_event_can_fund_statute() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="ipo_readiness_launchpad",
    ...
    product = state.products[0]
    state.company.current_turn = 38
    state.company.cash_on_hand = Decimal("7600.00")
    state.finance.board_confidence = 54
    state.finance.board_score = 50
    state.finance.board_pressure = 38
    state.finance.governance_risk = 34
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=37,
            resolved_turn=37,
            selected_option_id="fund_book_integrity_covenant",
            selected_option_label="Fund the book-integrity covenant",
            result_text="Book-integrity covenant funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="IPO Anchor Account",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4600.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "ipo_book_anchor_statute"
    )
    ...
    outcome = resolve_pending_event(state, "fund_book_anchor_statute")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.finance.board_score > state.finance.board_score
    assert outcome.state.finance.board_pressure < state.finance.board_pressure
    assert outcome.history_entry.event_id == "ipo_book_anchor_statute"


def test_buyer_close_anchor_statute_event_can_fund_statute() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="acquisition_diligence_sprint",
    ...
    product = state.products[0]
    state.company.current_turn = 38
    state.company.cash_on_hand = Decimal("7200.00")
    state.finance.board_confidence = 54
    state.finance.board_score = 50
    state.finance.board_pressure = 40
    state.finance.governance_risk = 34
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=37,
            resolved_turn=37,
            selected_option_id="fund_close_integrity_covenant",
            selected_option_label="Fund the close-integrity covenant",
            result_text="Close-integrity covenant funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Close Anchor Account",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4600.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    state.partnerships = [
        PartnershipDeal(
            ...
            conflict_pressure=70,
            enablement_level=18,
            sourced_revenue=Decimal("5000.00"),
            rev_share_rate=Decimal("0.2450"),
        )
    ]
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "buyer_close_anchor_statute"
    )
    ...
    outcome = resolve_pending_event(state, "fund_close_anchor_statute")
    ...
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.history_entry.event_id == "buyer_close_anchor_statute"


def test_independence_cash_solvency_statute_event_can_ratify_statute() -> None:
    product = make_product("Cash Solvency Statute Core", lifecycle_stage=LifecycleStage.MATURE)
    state = make_state(
        product,
        cash_on_hand=Decimal("2300.00"),
        current_turn=35,
        finance=FinanceState(
            debt_principal=Decimal("4800.00"),
            loan_interest_rate=Decimal("0.0310"),
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=34,
                resolved_turn=34,
                selected_option_id="ratify_cash_backstop_covenant",
                selected_option_label="Ratify the cash-backstop covenant",
                result_text="Cash-backstop covenant ratified.",
            )
        ],
    )
    state.capital_plan.reserve_target = Decimal("7200.00")
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "independence_cash_solvency_statute"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_cash_solvency_statute")
    ...
    assert outcome.state.finance.investor_pressure < state.finance.investor_pressure
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.history_entry.event_id == "independence_cash_solvency_statute"


def test_reseller_service_statute_event_can_fund_statute() -> None:
    product = make_product("Service Statute Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Service Statute Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2200.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=25,
                resolved_turn=25,
                selected_option_id="fund_service_backstop",
                selected_option_label="Fund the service backstop",
                result_text="Service backstop funded.",
            )
        ],
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "reseller_service_statute"
    )
    ...
    outcome = resolve_pending_event(state, "fund_service_statute")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "reseller_service_statute"


def test_integration_cutover_statute_event_can_fund_statute() -> None:
    product = make_product("Cutover Statute Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Cutover Statute Account",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3600.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Cutover Statute Integration",
        product_id=product.id,
        channel=PartnerChannel.INTEGRATION,
        status=PartnershipStatus.RECOVERY,
        quality=58,
        risk=76,
        conflict_pressure=72,
        enablement_level=20,
        sourced_revenue=Decimal("4600.00"),
        rev_share_rate=Decimal("0.2250"),
    )
    state = make_state(
        product,
        customer_accounts=[account],
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=26,
                resolved_turn=26,
                selected_option_id="fund_cutover_backstop",
                selected_option_label="Fund the cutover backstop",
                result_text="Cutover backstop funded.",
            )
        ],
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "integration_cutover_statute"
    )
    ...
    outcome = resolve_pending_event(state, "fund_cutover_statute")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "integration_cutover_statute"


def test_marketplace_refund_statute_event_can_fund_statute() -> None:
    product = make_product("Refund Statute Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Refund Statute Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2400.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Refund Statute Marketplace",
        product_id=product.id,
        channel=PartnerChannel.MARKETPLACE,
        status=PartnershipStatus.RECOVERY,
        quality=56,
        ...
        conflict_pressure=68,
        enablement_level=18,
        sourced_revenue=Decimal("4300.00"),
        rev_share_rate=Decimal("0.2300"),
    )
    state = make_state(
        product,
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=27,
                resolved_turn=27,
                selected_option_id="fund_refund_covenant",
                selected_option_label="Fund the refund covenant",
                result_text="Refund covenant funded.",
            )
        ],
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "marketplace_refund_statute"
    )
    ...
    outcome = resolve_pending_event(state, "fund_refund_statute")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.history_entry.event_id == "marketplace_refund_statute"


def test_board_reset_operating_statute_event_can_ratify_statute() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="board_recovery_crucible",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=35,
            resolved_turn=35,
            selected_option_id="ratify_execution_compact",
            selected_option_label="Ratify the execution compact",
            result_text="Execution compact ratified.",
        )
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "board_reset_operating_statute"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_operating_statute")
    ...
    assert outcome.state.finance.governance_risk < state.finance.governance_risk
    assert outcome.state.capital_plan.reserve_share > state.capital_plan.reserve_share
    assert outcome.history_entry.event_id == "board_reset_operating_statute"


def test_five_hundred_turn_board_recovery_crucible_progression_is_seed_stable() -> None:
    def run_once(seed: int) -> tuple[Decimal, int, int, bool, bool, int, int, str | None]:
        state = create_new_game(
            DEFAULT_COMPANY_NAME,
            DEFAULT_PRODUCT_NAME,
    ...
    assert run_once(5001) == run_once(5001)


def test_run_enterprise_reference_authority_rebuilds_terminal_flagship_trust() -> None:
    product = make_product("Reference Authority Core", target_segment=MarketSegment.ENTERPRISE)
    account = CustomerAccount(
        name="Reference Authority Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("52000.00"))
    state.support_program.backlog_queue = 30
...
def test_run_billing_liquidity_authority_cools_terminal_collections_extreme_heat() -> None:
    product = make_product("Billing Authority Core", target_segment=MarketSegment.SMB)
    account = CustomerAccount(
        name="Liquidity Authority Anchor",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("6100.00"),
        support_tier=SupportTier.PRIORITY,
        satisfaction=22,
        onboarding_health=40,
        support_load=64,
        ...
        churn_risk=58,
        invoice_risk=86,
        failed_payment_risk=80,
        dunning_steps=13,
        escalation_count=10,
        status=CustomerAccountStatus.AT_RISK,
    )
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("47000.00"))
    state.support_program.backlog_queue = 26
    state.support_program.escalation_queue = 13
    state.finance.board_pressure = 46
    state.finance.investor_pressure = 40
    state.finance.covenant_risk = 38
    state.finance.board_confidence = 18
...
def test_run_onboarding_continuity_authority_stabilizes_terminal_drag() -> None:
    product = make_product(
        "Onboarding Continuity Authority Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Continuity Authority Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("6400.00"),
        support_tier=SupportTier.WHITE_GLOVE,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("43800.00"))
    state.support_program.backlog_queue = 28
    state.support_program.escalation_queue = 13
    state.finance.board_pressure = 42
    state.finance.board_confidence = 24
...
def test_run_channel_durability_mandate_recovers_terminal_hotspot_channel() -> None:
    product = make_product(
        "Channel Durability Mandate Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Durability Mandate Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3600.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Durability Mandate Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.STRAINED,
        quality=40,
        risk=86,
        conflict_pressure=82,
        enablement_level=6,
        sourced_revenue=Decimal("8100.00"),
        sourced_users=94,
        rev_share_rate=Decimal("0.2950"),
    )
    state = make_state(
        product,
        customer_accounts=[account],
        partnerships=[partnership],
        cash_on_hand=Decimal("46800.00"),
        current_turn=26,
    )
    state.finance.board_pressure = 46
    state.finance.investor_pressure = 40
...
def test_set_terminal_solvency_mandate_shifts_to_deepest_terminal_controls() -> None:
    product = make_product(
        "Terminal Solvency Mandate Core", target_segment=MarketSegment.ENTERPRISE
    )
    capital_plan = CapitalPlan(
        mode=CapitalPlanMode.BALANCED,
        source_preference=CapitalSourcePreference.VENTURE,
    ...
    account = CustomerAccount(
        name="Terminal Solvency Mandate Account",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("5800.00"),
        support_tier=SupportTier.WHITE_GLOVE,
        satisfaction=40,
        onboarding_health=30,
        support_load=58,
        open_tickets=16,
        sla_breach_risk=78,
        renewal_health=18,
    ...
    partnership = PartnershipDeal(
        name="Terminal Solvency Mandate Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.RECOVERY,
        quality=44,
    ...
    state.finance.governance_risk = 70
    state.finance.covenant_risk = 36
    state.finance.board_confidence = 20
    state.finance.investor_pressure = 18
    state.finance.board_warning_level = 2
    state.finance.restructuring_pressure = 34
    ...
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert "terminal solvency mandate" in outcome.message


def test_finance_planner_recommends_enterprise_reference_authority_for_flagship_heat() -> None:
    state = make_state(
        make_product("Enterprise Reference Authority Planning Core"),
        cash_on_hand=Decimal("4200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        support_backlog=24,
        support_escalations=9,
        premium_revenue_at_risk_value=Decimal("6200.00"),
        enterprise_queue_risk_accounts=3,
        high_value_risk_accounts=2,
        white_glove_queue_risk_accounts=1,
        revenue_at_risk_value=Decimal("5600.00"),
        renewal_queue_risk_accounts=2,
        renewal_pressure_value=Decimal("3200.00"),
        support_lane_focus=SupportLaneFocus.BILLING,
        support_hotspot_lane=SupportLaneFocus.ENTERPRISE,
    ...
    assert "run_enterprise_reference_authority" in planner.recommended_actions
    assert any("enterprise reference authority" in step for step in planner.action_sequence)


def test_finance_planner_recommends_billing_liquidity_authority_for_terminal_covenant_heat() -> (
    None
):
    state = make_state(
        make_product("Billing Liquidity Authority Planning Core"),
        cash_on_hand=Decimal("3400.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        revenue_at_risk_value=Decimal("5200.00"),
        renewal_queue_risk_accounts=3,
        renewal_pressure_value=Decimal("6800.00"),
        support_lane_focus=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane=SupportLaneFocus.BILLING,
        support_hotspot_lane_overflow=7,
        hotspot_lane_account_count=4,
    ...
    assert "run_billing_liquidity_authority" in planner.recommended_actions
    assert any("billing liquidity authority" in step for step in planner.action_sequence)


def test_finance_planner_recommends_onboarding_continuity_authority_for_terminal_drag() -> None:
    state = make_state(
        make_product("Onboarding Continuity Authority Planning Core"),
        cash_on_hand=Decimal("3900.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    planner = build_finance_planner(
        ...
        revenue_at_risk_value=Decimal("6000.00"),
        renewal_pressure_value=Decimal("5000.00"),
        support_lane_focus=SupportLaneFocus.BALANCED,
        support_hotspot_lane=SupportLaneFocus.ONBOARDING,
    ...
    assert "run_onboarding_continuity_authority" in planner.recommended_actions
    assert any("onboarding continuity authority" in step for step in planner.action_sequence)


def test_finance_planner_recommends_channel_durability_mandate_for_terminal_dependency_heat() -> (
    None
):
    state = make_state(
        make_product("Channel Durability Mandate Planning Core"),
        cash_on_hand=Decimal("3600.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        hotspot_lane_account_count=4,
        focus_alignment_gap=3,
        channel_conflict_index=48,
        paused_dependency_score=96,
        hotspot_dependency_score=136,
        recovery_drag_score=122,
        hotspot_revenue_share_percent=62,
        hotspot_channel="reseller",
        strategic_outlook="strategic_acquisition",
        dominant_endgame_pressure="acquirer_diligence",
        commercial_fragility=92,
    ...
    assert "run_channel_durability_mandate" in planner.recommended_actions
    assert any("channel durability mandate" in step for step in planner.action_sequence)


def test_finance_planner_recommends_terminal_solvency_mandate_for_board_reset_heat() -> None:
    state = make_state(
        make_product("Terminal Solvency Mandate Planning Core"),
        cash_on_hand=Decimal("3200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-1320.00"),
        capital_plan=state.capital_plan,
        support_backlog=26,
        support_escalations=10,
        support_lane_focus=SupportLaneFocus.BILLING,
        ...
        hotspot_lane_account_count=5,
        focus_alignment_gap=4,
        hotspot_dependency_score=132,
        strategic_outlook="board_reset",
        dominant_endgame_pressure="board_reset_risk",
        commercial_fragility=94,
        capital_fragility=92,
    )

    assert "set_terminal_solvency_mandate" in planner.recommended_actions
    assert any("terminal solvency mandate" in step for step in planner.action_sequence)


def test_ipo_book_anchor_mandate_event_can_fund_mandate() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="ipo_readiness_launchpad",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=38,
            resolved_turn=38,
            selected_option_id="fund_book_anchor_statute",
            selected_option_label="Fund the book-anchor statute",
            result_text="Book-anchor statute funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="IPO Mandate Account",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4800.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "ipo_book_anchor_mandate"
    )
    ...
    outcome = resolve_pending_event(state, "fund_book_anchor_mandate")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.finance.board_score > state.finance.board_score
    assert outcome.state.finance.board_pressure < state.finance.board_pressure
    assert outcome.history_entry.event_id == "ipo_book_anchor_mandate"


def test_buyer_close_anchor_mandate_event_can_fund_mandate() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="acquisition_diligence_sprint",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=38,
            resolved_turn=38,
            selected_option_id="fund_close_anchor_statute",
            selected_option_label="Fund the close-anchor statute",
            result_text="Close-anchor statute funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Close Mandate Account",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4700.00"),
            support_tier=SupportTier.WHITE_GLOVE,
            satisfaction=58,
            onboarding_health=54,
            support_load=36,
    ...
    state.partnerships = [
        PartnershipDeal(
            name="Close Mandate Integration",
            product_id=product.id,
            channel=PartnerChannel.INTEGRATION,
            status=PartnershipStatus.STRAINED,
            quality=56,
            ...
            enablement_level=18,
            sourced_revenue=Decimal("5200.00"),
            rev_share_rate=Decimal("0.2480"),
        )
    ]
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "buyer_close_anchor_mandate"
    )
    ...
    outcome = resolve_pending_event(state, "fund_close_anchor_mandate")
    ...
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.history_entry.event_id == "buyer_close_anchor_mandate"


def test_independence_cash_solvency_mandate_event_can_ratify_mandate() -> None:
    product = make_product("Cash Solvency Mandate Core", lifecycle_stage=LifecycleStage.MATURE)
    state = make_state(
        product,
        cash_on_hand=Decimal("2300.00"),
        current_turn=36,
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=35,
                resolved_turn=35,
                selected_option_id="ratify_cash_solvency_statute",
                selected_option_label="Ratify the cash-solvency statute",
                result_text="Cash-solvency statute ratified.",
            )
        ],
    )
    state.capital_plan.reserve_target = Decimal("7600.00")
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "independence_cash_solvency_mandate"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_cash_solvency_mandate")
    ...
    assert outcome.state.finance.investor_pressure < state.finance.investor_pressure
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.history_entry.event_id == "independence_cash_solvency_mandate"


def test_reseller_service_mandate_event_can_fund_mandate() -> None:
    product = make_product("Service Mandate Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Service Mandate Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2400.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        ...
        conflict_pressure=72,
        enablement_level=22,
        sourced_revenue=Decimal("4400.00"),
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=26,
                resolved_turn=26,
                selected_option_id="fund_service_statute",
                selected_option_label="Fund the service statute",
                result_text="Service statute funded.",
            )
        ],
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "reseller_service_mandate"
    )
    ...
    outcome = resolve_pending_event(state, "fund_service_mandate")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "reseller_service_mandate"


def test_integration_cutover_mandate_event_can_fund_mandate() -> None:
    product = make_product("Cutover Mandate Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Cutover Mandate Account",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3800.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Cutover Mandate Integration",
        product_id=product.id,
        channel=PartnerChannel.INTEGRATION,
        status=PartnershipStatus.RECOVERY,
        quality=58,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=27,
                resolved_turn=27,
                selected_option_id="fund_cutover_statute",
                selected_option_label="Fund the cutover statute",
                result_text="Cutover statute funded.",
            )
        ],
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "integration_cutover_mandate"
    )
    ...
    outcome = resolve_pending_event(state, "fund_cutover_mandate")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "integration_cutover_mandate"


def test_marketplace_refund_mandate_event_can_fund_mandate() -> None:
    product = make_product("Refund Mandate Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Refund Mandate Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2500.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Refund Mandate Marketplace",
        product_id=product.id,
        channel=PartnerChannel.MARKETPLACE,
        status=PartnershipStatus.RECOVERY,
        quality=56,
        ...
        conflict_pressure=70,
        enablement_level=18,
        sourced_revenue=Decimal("4500.00"),
        rev_share_rate=Decimal("0.2340"),
    )
    state = make_state(
        product,
        customer_accounts=[account],
        ...
        event_history=[
            EventHistoryEntry(
                ...
                triggered_turn=28,
                resolved_turn=28,
                selected_option_id="fund_refund_statute",
                selected_option_label="Fund the refund statute",
                result_text="Refund statute funded.",
            )
        ],
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "marketplace_refund_mandate"
    )
    ...
    outcome = resolve_pending_event(state, "fund_refund_mandate")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.history_entry.event_id == "marketplace_refund_mandate"


def test_board_reset_operating_mandate_event_can_ratify_mandate() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="board_recovery_crucible",
    ...
    state.event_history.append(
        EventHistoryEntry(
            ...
            triggered_turn=36,
            resolved_turn=36,
            selected_option_id="ratify_operating_statute",
            selected_option_label="Ratify the operating statute",
            result_text="Operating statute ratified.",
        )
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "board_reset_operating_mandate"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_operating_mandate")
    ...
    assert outcome.state.finance.governance_risk < state.finance.governance_risk
    assert outcome.state.capital_plan.reserve_share > state.capital_plan.reserve_share
    assert outcome.history_entry.event_id == "board_reset_operating_mandate"


def test_five_hundred_forty_turn_board_recovery_crucible_progression_is_seed_stable() -> None:
    def run_once(seed: int) -> tuple[Decimal, int, int, bool, bool, int, int, str | None]:
        state = create_new_game(
            DEFAULT_COMPANY_NAME,
            DEFAULT_PRODUCT_NAME,
    ...
    assert run_once(5401) == run_once(5401)


def test_run_enterprise_reference_commission_rebuilds_terminal_flagship_trust() -> None:
    product = make_product("Reference Commission Core", target_segment=MarketSegment.ENTERPRISE)
    account = CustomerAccount(
        name="Reference Commission Flagship",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("13800.00"),
        support_tier=SupportTier.WHITE_GLOVE,
        satisfaction=22,
        onboarding_health=16,
        support_load=68,
    ...
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("56000.00"))
    state.support_program.backlog_queue = 32
    state.support_program.escalation_queue = 16
    state.finance.board_pressure = 50
    state.finance.board_confidence = 16
    state.finance.board_score = 16
    state.finance.investor_pressure = 36
    state.company.reputation = 50
...
def test_run_billing_liquidity_commission_cools_terminal_collections_extreme_heat() -> None:
    product = make_product("Billing Commission Core", target_segment=MarketSegment.SMB)
    account = CustomerAccount(
        name="Liquidity Commission Anchor",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("6600.00"),
        support_tier=SupportTier.PRIORITY,
        satisfaction=20,
        onboarding_health=38,
        support_load=66,
        ...
        sla_breach_risk=76,
        ticket_queue_age=10,
        expansion_potential=32,
        renewal_health=12,
        renewal_turn=4,
        churn_risk=60,
        invoice_risk=88,
        failed_payment_risk=82,
        dunning_steps=14,
        escalation_count=11,
        status=CustomerAccountStatus.AT_RISK,
    )
    state = make_state(product, customer_accounts=[account], cash_on_hand=Decimal("50000.00"))
    state.support_program.backlog_queue = 28
    state.support_program.escalation_queue = 14
    state.finance.board_pressure = 48
    state.finance.investor_pressure = 42
    state.finance.covenant_risk = 40
    state.finance.board_confidence = 16
...
def test_run_onboarding_continuity_commission_stabilizes_terminal_drag() -> None:
    product = make_product(
        "Onboarding Continuity Commission Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Continuity Commission Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("6800.00"),
        support_tier=SupportTier.WHITE_GLOVE,
...
def test_run_channel_durability_commission_recovers_terminal_hotspot_channel() -> None:
    product = make_product(
        "Channel Durability Commission Core",
        target_segment=MarketSegment.ENTERPRISE,
    )
    account = CustomerAccount(
        name="Durability Commission Anchor",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("3900.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Durability Commission Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.STRAINED,
        quality=38,
        risk=88,
        conflict_pressure=84,
        enablement_level=4,
        sourced_revenue=Decimal("8600.00"),
        sourced_users=98,
        rev_share_rate=Decimal("0.3050"),
    )
    state = make_state(
        product,
        customer_accounts=[account],
        partnerships=[partnership],
        cash_on_hand=Decimal("49200.00"),
        current_turn=27,
    )
    state.finance.board_pressure = 48
    state.finance.investor_pressure = 42
...
def test_set_terminal_solvency_commission_shifts_to_deepest_terminal_controls() -> None:
    product = make_product(
        "Terminal Solvency Commission Core", target_segment=MarketSegment.ENTERPRISE
    )
    capital_plan = CapitalPlan(
        mode=CapitalPlanMode.BALANCED,
        source_preference=CapitalSourcePreference.VENTURE,
    ...
    account = CustomerAccount(
        name="Terminal Solvency Commission Account",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("6200.00"),
        support_tier=SupportTier.WHITE_GLOVE,
    ...
    partnership = PartnershipDeal(
        name="Terminal Solvency Commission Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.RECOVERY,
        quality=42,
        ...
        conflict_pressure=74,
        enablement_level=10,
        sourced_revenue=Decimal("6600.00"),
        sourced_users=82,
        rev_share_rate=Decimal("0.2820"),
    )
    state = make_state(
        product,
        customer_accounts=[account],
    ...
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert "terminal solvency commission" in outcome.message


def test_finance_planner_recommends_enterprise_reference_commission_for_flagship_heat() -> None:
    state = make_state(
        make_product("Enterprise Reference Commission Planning Core"),
        cash_on_hand=Decimal("3900.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        high_value_risk_accounts=2,
        white_glove_queue_risk_accounts=1,
        revenue_at_risk_value=Decimal("5800.00"),
        renewal_queue_risk_accounts=2,
        renewal_pressure_value=Decimal("3400.00"),
        support_lane_focus=SupportLaneFocus.BILLING,
        support_hotspot_lane=SupportLaneFocus.ENTERPRISE,
    ...
    assert "run_enterprise_reference_commission" in planner.recommended_actions
    assert any("enterprise reference commission" in step for step in planner.action_sequence)


def test_finance_planner_recommends_billing_liquidity_commission_for_terminal_covenant_heat() -> (
    None
):
    state = make_state(
        make_product("Billing Liquidity Commission Planning Core"),
        cash_on_hand=Decimal("3200.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    state.finance.covenant_risk = 40
    state.finance.board_pressure = 46
    state.finance.debt_principal = Decimal("5600.00")
    planner = build_finance_planner(
        state.company,
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-1300.00"),
        capital_plan=state.capital_plan,
        support_backlog=28,
        support_escalations=11,
        revenue_at_risk_value=Decimal("5400.00"),
        ...
        dominant_endgame_pressure="independence_discipline",
        commercial_fragility=90,
        capital_fragility=94,
    )

    assert "run_billing_liquidity_commission" in planner.recommended_actions
    assert any("billing liquidity commission" in step for step in planner.action_sequence)


def test_finance_planner_recommends_onboarding_continuity_commission_for_terminal_drag() -> None:
    state = make_state(
        make_product("Onboarding Continuity Commission Planning Core"),
        cash_on_hand=Decimal("3600.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.ANGEL,
    ...
    planner = build_finance_planner(
        ...
        revenue_at_risk_value=Decimal("6400.00"),
        renewal_pressure_value=Decimal("5400.00"),
        support_lane_focus=SupportLaneFocus.BALANCED,
        support_hotspot_lane=SupportLaneFocus.ONBOARDING,
        support_hotspot_lane_overflow=11,
        hotspot_lane_account_count=6,
        focus_alignment_gap=4,
        strategic_outlook="ipo_ready",
        dominant_endgame_pressure="public_market_scrutiny",
    ...
    assert "run_onboarding_continuity_commission" in planner.recommended_actions
    assert any("onboarding continuity commission" in step for step in planner.action_sequence)


def test_finance_planner_recommends_channel_durability_commission_for_dependency_heat() -> None:
    state = make_state(
        make_product("Channel Durability Commission Planning Core"),
        cash_on_hand=Decimal("3300.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-1220.00"),
        capital_plan=state.capital_plan,
        support_backlog=26,
        support_escalations=10,
        support_lane_focus=SupportLaneFocus.BILLING,
        ...
        hotspot_lane_account_count=4,
        focus_alignment_gap=3,
        channel_conflict_index=50,
        paused_dependency_score=100,
        hotspot_dependency_score=140,
        recovery_drag_score=126,
        hotspot_revenue_share_percent=66,
        hotspot_channel="reseller",
        strategic_outlook="strategic_acquisition",
        dominant_endgame_pressure="acquirer_diligence",
        commercial_fragility=94,
    ...
    assert "run_channel_durability_commission" in planner.recommended_actions
    assert any("channel durability commission" in step for step in planner.action_sequence)


def test_finance_planner_recommends_terminal_solvency_commission_for_board_reset_heat() -> None:
    state = make_state(
        make_product("Terminal Solvency Commission Planning Core"),
        cash_on_hand=Decimal("3000.00"),
        capital_plan=CapitalPlan(
            mode=CapitalPlanMode.EXPAND,
            source_preference=CapitalSourcePreference.DEBT,
    ...
    planner = build_finance_planner(
        ...
        state.finance,
        state.turn_history,
        latest_net_cash_flow=Decimal("-1360.00"),
        capital_plan=state.capital_plan,
        support_backlog=28,
        support_escalations=11,
        support_lane_focus=SupportLaneFocus.BILLING,
    ...
    assert "set_terminal_solvency_commission" in planner.recommended_actions
    assert any("terminal solvency commission" in step for step in planner.action_sequence)


def test_ipo_book_anchor_commission_event_can_fund_commission() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="ipo_readiness_launchpad",
    ...
    product = state.products[0]
    state.company.current_turn = 40
    state.company.cash_on_hand = Decimal("7800.00")
    state.finance.board_confidence = 56
    state.finance.board_score = 52
    state.finance.board_pressure = 36
    state.finance.governance_risk = 32
    state.event_history.append(
        EventHistoryEntry(
            event_id="ipo_book_anchor_mandate",
            category=EventCategory.FUNDING_OPPORTUNITY,
            title="IPO Book-Anchor Mandate",
            triggered_turn=39,
            resolved_turn=39,
            selected_option_id="fund_book_anchor_mandate",
            selected_option_label="Fund the book-anchor mandate",
            result_text="Book-anchor mandate funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="IPO Commission Account",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("5000.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "ipo_book_anchor_commission"
    )
    ...
    outcome = resolve_pending_event(state, "fund_book_anchor_commission")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.state.finance.board_score > state.finance.board_score
    assert outcome.state.finance.board_pressure < state.finance.board_pressure
    assert outcome.history_entry.event_id == "ipo_book_anchor_commission"


def test_buyer_close_anchor_commission_event_can_fund_commission() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="acquisition_diligence_sprint",
    ...
    product = state.products[0]
    state.company.current_turn = 40
    state.company.cash_on_hand = Decimal("7400.00")
    state.finance.board_confidence = 56
    state.finance.board_score = 52
    state.finance.board_pressure = 38
    state.finance.governance_risk = 32
    state.event_history.append(
        EventHistoryEntry(
            event_id="buyer_close_anchor_mandate",
            category=EventCategory.FUNDING_OPPORTUNITY,
            title="Buyer Close-Anchor Mandate",
            triggered_turn=39,
            resolved_turn=39,
            selected_option_id="fund_close_anchor_mandate",
            selected_option_label="Fund the close-anchor mandate",
            result_text="Close-anchor mandate funded.",
        )
    )
    state.customer_accounts = [
        CustomerAccount(
            name="Close Commission Account",
            product_id=product.id,
            segment=MarketSegment.ENTERPRISE,
            contract_value=Decimal("4800.00"),
            support_tier=SupportTier.WHITE_GLOVE,
    ...
    state.partnerships = [
        PartnershipDeal(
            name="Close Commission Integration",
            product_id=product.id,
            channel=PartnerChannel.INTEGRATION,
            status=PartnershipStatus.STRAINED,
            quality=56,
    ...
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "buyer_close_anchor_commission"
    )
    ...
    outcome = resolve_pending_event(state, "fund_close_anchor_commission")
    ...
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.history_entry.event_id == "buyer_close_anchor_commission"


def test_independence_cash_solvency_commission_event_can_ratify_commission() -> None:
    product = make_product("Cash Solvency Commission Core", lifecycle_stage=LifecycleStage.MATURE)
    state = make_state(
        product,
        cash_on_hand=Decimal("2200.00"),
        current_turn=37,
        finance=FinanceState(
            debt_principal=Decimal("5600.00"),
            loan_interest_rate=Decimal("0.0310"),
            investor_pressure=38,
            covenant_risk=36,
            board_confidence=44,
            board_pressure=42,
        ),
        event_history=[
            EventHistoryEntry(
                event_id="independence_cash_solvency_mandate",
                category=EventCategory.FUNDING_OPPORTUNITY,
                title="Independence Cash-Solvency Mandate",
                triggered_turn=36,
                resolved_turn=36,
                selected_option_id="ratify_cash_solvency_mandate",
                selected_option_label="Ratify the cash-solvency mandate",
                result_text="Cash-solvency mandate ratified.",
            )
        ],
    )
    state.capital_plan.reserve_target = Decimal("7800.00")
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "independence_cash_solvency_commission"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_cash_solvency_commission")
    ...
    assert outcome.state.finance.investor_pressure < state.finance.investor_pressure
    assert outcome.state.finance.board_confidence > state.finance.board_confidence
    assert outcome.history_entry.event_id == "independence_cash_solvency_commission"


def test_reseller_service_commission_event_can_fund_commission() -> None:
    product = make_product("Service Commission Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Service Commission Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2500.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Service Commission Reseller",
        product_id=product.id,
        channel=PartnerChannel.RESELLER,
        status=PartnershipStatus.RECOVERY,
        quality=58,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                event_id="reseller_service_mandate",
                category=EventCategory.MARKET_OPPORTUNITY,
                title="Reseller Service Mandate",
                triggered_turn=27,
                resolved_turn=27,
                selected_option_id="fund_service_mandate",
                selected_option_label="Fund the service mandate",
                result_text="Service mandate funded.",
            )
        ],
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "reseller_service_commission"
    )
    ...
    outcome = resolve_pending_event(state, "fund_service_commission")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "reseller_service_commission"


def test_integration_cutover_commission_event_can_fund_commission() -> None:
    product = make_product("Cutover Commission Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Cutover Commission Account",
        product_id=product.id,
        segment=MarketSegment.ENTERPRISE,
        contract_value=Decimal("4000.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Cutover Commission Integration",
        product_id=product.id,
        channel=PartnerChannel.INTEGRATION,
        status=PartnershipStatus.RECOVERY,
        quality=58,
    ...
    state = make_state(
        ...
        event_history=[
            EventHistoryEntry(
                event_id="integration_cutover_mandate",
                category=EventCategory.PRODUCT_INCIDENT,
                title="Integration Cutover Mandate",
                triggered_turn=28,
                resolved_turn=28,
                selected_option_id="fund_cutover_mandate",
                selected_option_label="Fund the cutover mandate",
                result_text="Cutover mandate funded.",
            )
        ],
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "integration_cutover_commission"
    )
    ...
    outcome = resolve_pending_event(state, "fund_cutover_commission")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.state.partnerships[0].enablement_level > state.partnerships[0].enablement_level
    assert outcome.history_entry.event_id == "integration_cutover_commission"


def test_marketplace_refund_commission_event_can_fund_commission() -> None:
    product = make_product("Refund Commission Core", lifecycle_stage=LifecycleStage.MATURE)
    account = CustomerAccount(
        name="Refund Commission Account",
        product_id=product.id,
        segment=MarketSegment.SMB,
        contract_value=Decimal("2600.00"),
        support_tier=SupportTier.PRIORITY,
    ...
    partnership = PartnershipDeal(
        name="Refund Commission Marketplace",
        product_id=product.id,
        channel=PartnerChannel.MARKETPLACE,
        status=PartnershipStatus.RECOVERY,
        quality=56,
        ...
        conflict_pressure=72,
        enablement_level=18,
        sourced_revenue=Decimal("4700.00"),
        rev_share_rate=Decimal("0.2380"),
    )
    state = make_state(
        product,
        customer_accounts=[account],
        ...
        event_history=[
            EventHistoryEntry(
                event_id="marketplace_refund_mandate",
                category=EventCategory.REPUTATION_INCIDENT,
                title="Marketplace Refund Mandate",
                triggered_turn=29,
                resolved_turn=29,
                selected_option_id="fund_refund_mandate",
                selected_option_label="Fund the refund mandate",
                result_text="Refund mandate funded.",
            )
        ],
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "marketplace_refund_commission"
    )
    ...
    outcome = resolve_pending_event(state, "fund_refund_commission")

    assert outcome.state.company.cash_on_hand < state.company.cash_on_hand
    assert outcome.state.partnerships[0].risk < state.partnerships[0].risk
    assert outcome.state.partnerships[0].conflict_pressure < state.partnerships[0].conflict_pressure
    assert outcome.history_entry.event_id == "marketplace_refund_commission"


def test_board_reset_operating_commission_event_can_ratify_commission() -> None:
    state = create_new_game(
        DEFAULT_COMPANY_NAME,
        DEFAULT_PRODUCT_NAME,
        campaign_start_id="board_recovery_crucible",
    ...
    state.event_history.append(
        EventHistoryEntry(
            event_id="board_reset_operating_mandate",
            category=EventCategory.FUNDING_OPPORTUNITY,
            title="Board-Reset Operating Mandate",
            triggered_turn=37,
            resolved_turn=37,
            selected_option_id="ratify_operating_mandate",
            selected_option_label="Ratify the operating mandate",
            result_text="Operating mandate ratified.",
        )
    )
    definition = next(
        event_definition
        for event_definition in get_event_registry()
        if event_definition.event_id == "board_reset_operating_commission"
    )
    ...
    outcome = resolve_pending_event(state, "ratify_operating_commission")
    ...
    assert outcome.state.finance.governance_risk < state.finance.governance_risk
    assert outcome.state.capital_plan.reserve_share > state.capital_plan.reserve_share
    assert outcome.history_entry.event_id == "board_reset_operating_commission"


def test_five_hundred_eighty_turn_board_recovery_crucible_progression_is_seed_stable() -> None:
    def run_once(seed: int) -> tuple[Decimal, int, int, bool, bool, int, int, str | None]:
        state = create_new_game(
            DEFAULT_COMPANY_NAME,
            DEFAULT_PRODUCT_NAME,
    ...
    assert run_once(5801) == run_once(5801)
