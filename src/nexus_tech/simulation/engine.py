...
from nexus_tech.simulation.capital_planning import (
    ...
    apply_set_terminal_solvency_council,
...
def apply_action(
    ...
    if action is TurnAction.HARDEN_FINANCING_POSTURE:
        summary = apply_harden_financing_posture(next_state)
    ...
    if action is TurnAction.SET_TERMINAL_LIQUIDITY_CONTROLS:
        summary = apply_set_terminal_liquidity_controls(next_state)
        logger.debug("Set terminal liquidity controls.")
        return ActionOutcome(state=next_state, message=summary.message)

    if action is TurnAction.SET_CAPITAL_REALLOCATION_GRID:
        summary = apply_set_capital_reallocation_grid(next_state)
        logger.debug("Set capital reallocation grid.")
        return ActionOutcome(state=next_state, message=summary.message)

    if action is TurnAction.SET_PATH_CONTROL_MATRIX:
        summary = apply_set_path_control_matrix(next_state)
        logger.debug("Set path control matrix.")
        return ActionOutcome(state=next_state, message=summary.message)

    if action is TurnAction.SET_PATH_RESILIENCE_GRID:
        summary = apply_set_path_resilience_grid(next_state)
    ...
    if action is TurnAction.SET_TERMINAL_RECOVERY_LATTICE:
        summary = apply_set_terminal_recovery_lattice(next_state)
        logger.debug("Set terminal recovery lattice.")
        return ActionOutcome(state=next_state, message=summary.message)

    if action is TurnAction.SET_TERMINAL_CONTINUITY_MATRIX:
        summary = apply_set_terminal_continuity_matrix(next_state)
        logger.debug("Set terminal continuity matrix.")
        return ActionOutcome(state=next_state, message=summary.message)

    if action is TurnAction.SET_TERMINAL_RESILIENCE_COVENANT:
        summary = apply_set_terminal_resilience_covenant(next_state)
    ...
    if action is TurnAction.SET_TERMINAL_SOLVENCY_COUNCIL:
        summary = apply_set_terminal_solvency_council(next_state)
        logger.debug("Set terminal solvency council.")
        return ActionOutcome(state=next_state, message=summary.message)
    ...
    if action is TurnAction.RUN_ENTERPRISE_QUEUE_RESET:
        ...
        summary = run_enterprise_queue_reset(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran enterprise queue reset for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_WHITE_GLOVE_REFERENCE_BUREAU:
        ...
        summary = run_white_glove_reference_bureau(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran white-glove reference bureau for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ENTERPRISE_COMMITMENT_BOARD:
        ...
        summary = run_enterprise_commitment_board(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran enterprise commitment board for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ENTERPRISE_REFERENCE_CHAMBER:
        ...
        summary = run_enterprise_reference_chamber(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran enterprise reference chamber for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ENTERPRISE_REFERENCE_FORUM:
        ...
        summary = run_enterprise_reference_forum(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran enterprise reference forum for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ENTERPRISE_REFERENCE_LATTICE:
        ...
        summary = run_enterprise_reference_lattice(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran enterprise reference lattice for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ENTERPRISE_REFERENCE_SUMMIT:
        ...
        summary = run_enterprise_reference_summit(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran enterprise reference summit for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ENTERPRISE_REFERENCE_DIRECTORATE:
        ...
        summary = run_enterprise_reference_directorate(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran enterprise reference directorate for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ENTERPRISE_REFERENCE_AUTHORITY:
        ...
        summary = run_enterprise_reference_authority(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran enterprise reference authority for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ENTERPRISE_REFERENCE_COUNCIL:
        ...
        summary = run_enterprise_reference_council(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran enterprise reference council for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_BILLING_DISPUTE_CABINET:
        ...
        summary = run_billing_dispute_cabinet(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran billing dispute cabinet for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_BILLING_COLLECTION_BRIDGE:
        ...
        summary = run_billing_collection_bridge(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran billing collection bridge for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_BILLING_COLLECTION_OFFICE:
        ...
        summary = run_billing_collection_office(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran billing collection office for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_BILLING_SETTLEMENT_BOARD:
        ...
        summary = run_billing_settlement_board(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran billing settlement board for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_BILLING_LIQUIDITY_COMMAND:
        ...
        summary = run_billing_liquidity_command(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran billing liquidity command for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_BILLING_LIQUIDITY_SUMMIT:
        ...
        summary = run_billing_liquidity_summit(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran billing liquidity summit for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_BILLING_LIQUIDITY_DIRECTORATE:
        ...
        summary = run_billing_liquidity_directorate(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran billing liquidity directorate for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_BILLING_LIQUIDITY_AUTHORITY:
        ...
        summary = run_billing_liquidity_authority(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran billing liquidity authority for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_BILLING_LIQUIDITY_COUNCIL:
        ...
        summary = run_billing_liquidity_council(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran billing liquidity council for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ONBOARDING_LAUNCH_CELL:
        ...
        summary = run_onboarding_launch_cell(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran onboarding launch cell for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ONBOARDING_ADOPTION_HUB:
        ...
        summary = run_onboarding_adoption_hub(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran onboarding adoption hub for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ONBOARDING_STABILITY_BOARD:
        ...
        summary = run_onboarding_stability_board(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran onboarding stability board for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ONBOARDING_DURABILITY_MESH:
        ...
        summary = run_onboarding_durability_mesh(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran onboarding durability mesh for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ONBOARDING_CONTINUITY_LATTICE:
        ...
        summary = run_onboarding_continuity_lattice(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran onboarding continuity lattice for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_ONBOARDING_CONTINUITY_COUNCIL:
        ...
        summary = run_onboarding_continuity_council(next_state, account.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran onboarding continuity council for %s.", account.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_CHANNEL_SYNERGY_RESET:
        if context.partnership_id is None:
            raise ValueError("Running a channel synergy reset requires selecting a partnership.")
        partnership = get_partnership_by_id(next_state.partnerships, context.partnership_id)
    ...
    if action is TurnAction.RUN_CHANNEL_DEPENDENCY_RESET:
        if context.partnership_id is None:
            raise ValueError("Running a channel dependency reset requires selecting a partnership.")
        partnership = get_partnership_by_id(next_state.partnerships, context.partnership_id)
        summary = run_channel_dependency_reset(next_state, partnership.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran channel dependency reset for %s.", partnership.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_CHANNEL_CONFIDENCE_FIREWALL:
        if context.partnership_id is None:
            raise ValueError(
                "Running a channel confidence firewall requires selecting a partnership."
            )
        partnership = get_partnership_by_id(next_state.partnerships, context.partnership_id)
        summary = run_channel_confidence_firewall(next_state, partnership.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran channel confidence firewall for %s.", partnership.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_CHANNEL_DURABILITY_MESH:
        if context.partnership_id is None:
            raise ValueError("Running a channel durability mesh requires selecting a partnership.")
        partnership = get_partnership_by_id(next_state.partnerships, context.partnership_id)
    ...
    if action is TurnAction.RUN_CHANNEL_RESILIENCE_GRID:
        if context.partnership_id is None:
            raise ValueError("Running a channel resilience grid requires selecting a partnership.")
        partnership = get_partnership_by_id(next_state.partnerships, context.partnership_id)
        summary = run_channel_resilience_grid(next_state, partnership.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran channel resilience grid for %s.", partnership.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_CHANNEL_CONTINUITY_MATRIX:
        if context.partnership_id is None:
            raise ValueError(
                "Running a channel continuity matrix requires selecting a partnership."
            )
        partnership = get_partnership_by_id(next_state.partnerships, context.partnership_id)
        summary = run_channel_continuity_matrix(next_state, partnership.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran channel continuity matrix for %s.", partnership.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
    ...
    if action is TurnAction.RUN_CHANNEL_ASSURANCE_COVENANT:
        if context.partnership_id is None:
            raise ValueError(
                "Running a channel assurance covenant requires selecting a partnership."
            )
        partnership = get_partnership_by_id(next_state.partnerships, context.partnership_id)
    ...
    if action is TurnAction.RUN_CHANNEL_DURABILITY_MANDATE:
        if context.partnership_id is None:
            raise ValueError(
                "Running a channel durability mandate requires selecting a partnership."
            )
        partnership = get_partnership_by_id(next_state.partnerships, context.partnership_id)
    ...
    if action is TurnAction.RUN_CHANNEL_DURABILITY_COUNCIL:
        if context.partnership_id is None:
            raise ValueError(
                "Running a channel durability council requires selecting a partnership."
            )
        partnership = get_partnership_by_id(next_state.partnerships, context.partnership_id)
        summary = run_channel_durability_council(next_state, partnership.id)
        next_state.company.game_over = is_game_over(next_state.company)
        logger.debug("Ran channel durability council for %s.", partnership.name)
        return ActionOutcome(
            state=next_state,
            message=summary.message,
            turn_should_end=next_state.company.game_over,
...
def apply_end_of_turn_commercial_pressure(
    ...
    *,
    ...
    if support_summary.white_glove_revenue_at_risk_value > Decimal("0.00"):
        state.finance.board_reliability_score = clamp_int(
            state.finance.board_reliability_score
            - min(
                3,
                int(
                    (
                        support_summary.white_glove_revenue_at_risk_value
                        / BALANCE.commercial_pressure_revenue_at_risk_value_divisor
                    ).to_integral_value()
                ),
            )
        )
    if support_summary.service_tier_pressure > 0:
        state.finance.board_team_health_score = clamp_int(
            state.finance.board_team_health_score
            - min(
                4,
                support_summary.service_tier_pressure
    ...
    if channel_economics_pressure:
        state.finance.board_profitability_score = clamp_int(
            state.finance.board_profitability_score
            - BALANCE.commercial_pressure_channel_economics_profitability_loss
        )
    ...
    if support_summary.severe_queue_accounts > 0:
        summary_parts.append(
