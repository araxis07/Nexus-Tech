...
def apply_set_path_cash_waterfall(state: GameState) -> CapitalPlanSummary:
    """Re-anchor allocation around the currently dominant endgame path."""
    ...
    if (
        pressure.board_reset_risk < 56
        and pressure.public_market_scrutiny < 56
        and pressure.acquirer_diligence < 56
        and pressure.independence_discipline < 56
        and state.finance.board_pressure < 20
        ...
        raise ValueError("There is not enough late-game strain to justify a path cash waterfall.")
    ...
    if state.company.cash_on_hand < cost:
        raise ValueError("Not enough cash to set a path cash waterfall this turn.")

    capital_plan = state.capital_plan
    dominant_path = pressure.dominant_pressure
    waterfall_note = readiness.strategic_outlook.replace("_", " ")
    if dominant_path == "board_reset_risk":
        waterfall_note = "board reset"
    ...
    planning_horizon_turns = min(
        12,
        capital_plan.planning_horizon_turns
        + BALANCE.capital_plan_endgame_capital_map_horizon_gain
        + BALANCE.capital_plan_path_capital_posture_horizon_gain,
    )
    product_share = capital_plan.product_investment_share
    go_to_market_share = capital_plan.go_to_market_share
    ...
    if dominant_path == "board_reset_risk":
        source_preference = CapitalSourcePreference.BOOTSTRAP
        reserve_share += 6
        go_to_market_share = max(0, go_to_market_share - 6)
        ...
        mode = (
            CapitalPlanMode.CONSERVE
            if pressure.public_market_scrutiny >= 72 or state.finance.board_pressure >= 42
            else CapitalPlanMode.BALANCED
        )
        waterfall_note = "IPO readiness"
    elif readiness.strategic_outlook == "strategic_acquisition":
        if source_preference is CapitalSourcePreference.DEBT:
            source_preference = CapitalSourcePreference.ANGEL
        reserve_share += 4
        product_share += 1
        go_to_market_share = max(0, go_to_market_share - 5)
        mode = (
            CapitalPlanMode.CONSERVE
            if pressure.acquirer_diligence >= 72 or portfolio.channel_conflict_index >= 44
            else CapitalPlanMode.BALANCED
        )
        waterfall_note = "buyer-close"
    else:
        source_preference = CapitalSourcePreference.BOOTSTRAP
        reserve_share += 5
        go_to_market_share = max(0, go_to_market_share - 5)
    ...
    if (
        ...
        reserve_share += 2
        go_to_market_share = max(0, go_to_market_share - 2)
    if portfolio.hotspot_revenue_share_percent >= BALANCE.finance_planner_volatile_share_threshold:
        reserve_share += 1
        go_to_market_share = max(0, go_to_market_share - 1)
    ...
    return CapitalPlanSummary(
        message=(
            f"Set a path cash waterfall for the {waterfall_note} story. Reserve target "
            f"{format_money(reserve_target)} over {planning_horizon_turns} turns. Allocation now "
            f"P {product_share}% / GTM {go_to_market_share}% / Reserve {reserve_share}% with "
            f"{source_preference.value} capital."
        ),
...
def apply_set_terminal_solvency_statute(state: GameState) -> CapitalPlanSummary:
    """Apply the terminal solvency statute when covenant-level controls still are not enough."""
    ...
    base_cost = quantize_money(
        BALANCE.capital_plan_endgame_capital_map_cost * 2
        + BALANCE.capital_plan_path_capital_posture_cost * 7
        + BALANCE.capital_plan_growth_firebreak_cost * 7
    )
    extra_cost = quantize_money(
        BALANCE.capital_plan_growth_firebreak_cost * 2
        + BALANCE.capital_plan_path_capital_posture_cost * 2
    ...
    if (
        pressure.board_reset_risk < 98
        and pressure.public_market_scrutiny < 98
        and pressure.acquirer_diligence < 98
        and pressure.independence_discipline < 98
        and state.finance.governance_risk < 66
        and state.finance.covenant_risk < 32
        and reserve_gap <= Decimal("0.00")
        and queue_exposure.hotspot_lane_overflow <= 7
        and portfolio.hotspot_dependency_score
        < BALANCE.finance_planner_reactivate_dependency_threshold + 20
    ):
        raise ValueError(
            "There is not enough terminal multi-path fragility to justify a solvency statute."
        )
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to set a terminal solvency statute this turn.")

    apply_set_terminal_resilience_covenant(state)
    capital_plan = state.capital_plan
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    reserve_target = quantize_money(
        capital_plan.reserve_target + BALANCE.capital_plan_growth_firebreak_target_step
    ...
    if (
        pressure.board_reset_risk >= 98
        or pressure.dominant_pressure == "board_reset_risk"
        or state.finance.governance_risk >= 66
    ):
        source_preference = CapitalSourcePreference.BOOTSTRAP
        reserve_share += 3
        go_to_market_share = max(0, go_to_market_share - 1)
    ...
    if queue_exposure.hotspot_lane_overflow > 5:
        reserve_share += 1
        go_to_market_share = max(0, go_to_market_share - 1)
    if queue_exposure.enterprise_queue_risk_accounts > 1:
        reserve_share += 1
    ...
    if (
        portfolio.hotspot_dependency_score
        >= BALANCE.finance_planner_reactivate_dependency_threshold + 14
        or portfolio.paused_dependency_score
        >= BALANCE.finance_planner_reactivate_dependency_threshold + 10
    ):
        reserve_share += 2
    ...
    return CapitalPlanSummary(
        message=(
            f"Set a terminal solvency statute for the {control_note} path. Reserve target "
            f"{format_money(reserve_target)} over {planning_horizon_turns} turns. Allocation now "
            f"P {product_share}% / GTM {go_to_market_share}% / Reserve {reserve_share}% with "
            f"{source_preference.value} capital."
        ),
...
def apply_set_terminal_solvency_mandate(state: GameState) -> CapitalPlanSummary:
    """Apply the deepest terminal solvency control after the statute tier saturates."""
    ...
    base_cost = quantize_money(
        BALANCE.capital_plan_endgame_capital_map_cost * 2
        + BALANCE.capital_plan_path_capital_posture_cost * 9
        + BALANCE.capital_plan_growth_firebreak_cost * 9
    )
    extra_cost = quantize_money(
        BALANCE.capital_plan_growth_firebreak_cost * 2
        + BALANCE.capital_plan_path_capital_posture_cost * 3
    ...
    if (
        pressure.board_reset_risk < 99
        and pressure.public_market_scrutiny < 99
        and pressure.acquirer_diligence < 99
        and pressure.independence_discipline < 99
        and state.finance.governance_risk < 68
        and state.finance.covenant_risk < 34
        and reserve_gap <= Decimal("0.00")
        and queue_exposure.hotspot_lane_overflow <= 8
        and portfolio.hotspot_dependency_score
        < BALANCE.finance_planner_reactivate_dependency_threshold + 22
    ):
        raise ValueError(
            "There is not enough terminal multi-path fragility to justify a solvency mandate."
        )
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to set a terminal solvency mandate this turn.")

    apply_set_terminal_solvency_statute(state)
    capital_plan = state.capital_plan
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    reserve_target = quantize_money(
        capital_plan.reserve_target + BALANCE.capital_plan_growth_firebreak_target_step
    ...
    if (
        pressure.board_reset_risk >= 99
        or pressure.dominant_pressure == "board_reset_risk"
        or state.finance.governance_risk >= 68
    ):
        source_preference = CapitalSourcePreference.BOOTSTRAP
        reserve_share += 3
        go_to_market_share = max(0, go_to_market_share - 1)
    ...
    state.finance.board_pressure = clamp_int(state.finance.board_pressure - 1)
    state.finance.governance_risk = clamp_int(state.finance.governance_risk - 1)
    state.finance.investor_pressure = clamp_int(state.finance.investor_pressure - 1)
    state.finance.covenant_risk = clamp_int(state.finance.covenant_risk - 1)
    state.finance.board_confidence = min(
        100,
    ...
    return CapitalPlanSummary(
        message=(
            f"Set a terminal solvency mandate for the {control_note} path. Reserve target "
            f"{format_money(reserve_target)} over {planning_horizon_turns} turns. Allocation now "
            f"P {product_share}% / GTM {go_to_market_share}% / Reserve {reserve_share}% with "
            f"{source_preference.value} capital."
        ),
...
def apply_set_terminal_solvency_commission(state: GameState) -> CapitalPlanSummary:
    """Apply the last terminal solvency control after the mandate tier saturates."""
    ...
    base_cost = quantize_money(
        BALANCE.capital_plan_endgame_capital_map_cost * 2
        + BALANCE.capital_plan_path_capital_posture_cost * 10
        + BALANCE.capital_plan_growth_firebreak_cost * 10
    )
    extra_cost = quantize_money(
        BALANCE.capital_plan_growth_firebreak_cost * 2
        + BALANCE.capital_plan_path_capital_posture_cost * 3
    ...
    if (
        pressure.board_reset_risk < 100
        and pressure.public_market_scrutiny < 100
        and pressure.acquirer_diligence < 100
        and pressure.independence_discipline < 100
        and state.finance.governance_risk < 70
        and state.finance.covenant_risk < 36
        and reserve_gap <= Decimal("0.00")
        and queue_exposure.hotspot_lane_overflow <= 9
        and portfolio.hotspot_dependency_score
        < BALANCE.finance_planner_reactivate_dependency_threshold + 24
    ):
        raise ValueError(
            "There is not enough terminal multi-path fragility to justify a solvency commission."
        )
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to set a terminal solvency commission this turn.")

    apply_set_terminal_solvency_mandate(state)
    capital_plan = state.capital_plan
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    reserve_target = quantize_money(
        capital_plan.reserve_target + BALANCE.capital_plan_growth_firebreak_target_step
    ...
    if (
        pressure.board_reset_risk >= 100
        or pressure.dominant_pressure == "board_reset_risk"
        or state.finance.governance_risk >= 70
    ):
        source_preference = CapitalSourcePreference.BOOTSTRAP
        reserve_share += 3
        go_to_market_share = max(0, go_to_market_share - 1)
    ...
    if queue_exposure.hotspot_lane_overflow > 7:
        reserve_share += 1
        go_to_market_share = max(0, go_to_market_share - 1)
    if queue_exposure.enterprise_queue_risk_accounts > 1:
        reserve_share += 1
    ...
    return CapitalPlanSummary(
        message=(
            f"Set a terminal solvency commission for the {control_note} path. Reserve target "
            f"{format_money(reserve_target)} over {planning_horizon_turns} turns. Allocation now "
            f"P {product_share}% / GTM {go_to_market_share}% / Reserve {reserve_share}% with "
            f"{source_preference.value} capital."
        ),
...
def apply_set_terminal_solvency_oversight(state: GameState) -> CapitalPlanSummary:
    """Apply one more terminal solvency control after the commission tier saturates."""
    ...
    base_cost = quantize_money(
        BALANCE.capital_plan_endgame_capital_map_cost * 2
        + BALANCE.capital_plan_path_capital_posture_cost * 13
        + BALANCE.capital_plan_growth_firebreak_cost * 12
    )
    extra_cost = quantize_money(
        BALANCE.capital_plan_growth_firebreak_cost * 2
        + BALANCE.capital_plan_path_capital_posture_cost * 3
    ...
    if (
        pressure.board_reset_risk < 112
        and pressure.public_market_scrutiny < 112
        and pressure.acquirer_diligence < 112
        and pressure.independence_discipline < 112
        and state.finance.governance_risk < 74
        and state.finance.covenant_risk < 40
        and reserve_gap <= Decimal("0.00")
        and queue_exposure.hotspot_lane_overflow <= 11
        and portfolio.hotspot_dependency_score
        < BALANCE.finance_planner_reactivate_dependency_threshold + 28
    ):
        raise ValueError(
            "There is not enough terminal multi-path fragility to justify a solvency oversight."
        )
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to set a terminal solvency oversight this turn.")

    apply_set_terminal_solvency_commission(state)
    capital_plan = state.capital_plan
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    reserve_target = quantize_money(
        capital_plan.reserve_target + BALANCE.capital_plan_growth_firebreak_target_step
    ...
    if (
        pressure.board_reset_risk >= 112
        or pressure.dominant_pressure == "board_reset_risk"
        or state.finance.governance_risk >= 74
    ):
        source_preference = CapitalSourcePreference.BOOTSTRAP
        reserve_share += 3
        go_to_market_share = max(0, go_to_market_share - 1)
    ...
    if queue_exposure.hotspot_lane_overflow > 9:
        reserve_share += 1
        go_to_market_share = max(0, go_to_market_share - 1)
    if queue_exposure.enterprise_queue_risk_accounts > 1:
        reserve_share += 1
    ...
    return CapitalPlanSummary(
        message=(
            f"Set a terminal solvency oversight for the {control_note} path. Reserve target "
            f"{format_money(reserve_target)} over {planning_horizon_turns} turns. Allocation now "
            f"P {product_share}% / GTM {go_to_market_share}% / Reserve {reserve_share}% with "
            f"{source_preference.value} capital."
        ),
...
def apply_set_terminal_solvency_council(state: GameState) -> CapitalPlanSummary:
    """Apply the council-tier terminal solvency control after the oversight tier saturates."""
    ...
    base_cost = quantize_money(
        BALANCE.capital_plan_endgame_capital_map_cost * 2
        + BALANCE.capital_plan_path_capital_posture_cost * 16
        + BALANCE.capital_plan_growth_firebreak_cost * 14
    )
    extra_cost = quantize_money(
        BALANCE.capital_plan_growth_firebreak_cost * 3
        + BALANCE.capital_plan_path_capital_posture_cost * 4
    )
    total_cost = quantize_money(base_cost + extra_cost)

    if (
        pressure.board_reset_risk < 118
        and pressure.public_market_scrutiny < 118
        and pressure.acquirer_diligence < 118
        and pressure.independence_discipline < 118
        and state.finance.governance_risk < 78
        and state.finance.covenant_risk < 42
        and reserve_gap <= Decimal("0.00")
        and queue_exposure.hotspot_lane_overflow <= 12
        and portfolio.hotspot_dependency_score
        < BALANCE.finance_planner_reactivate_dependency_threshold + 32
    ):
        raise ValueError(
            "There is not enough terminal multi-path fragility to justify a solvency council."
        )
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to set a terminal solvency council this turn.")

    apply_set_terminal_solvency_oversight(state)
    capital_plan = state.capital_plan
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    reserve_target = quantize_money(
        capital_plan.reserve_target + BALANCE.capital_plan_growth_firebreak_target_step * 2
    )
    planning_horizon_turns = min(12, capital_plan.planning_horizon_turns + 1)
    product_share = max(0, capital_plan.product_investment_share - 1)
    go_to_market_share = max(0, capital_plan.go_to_market_share - 3)
    reserve_share = capital_plan.reserve_share + 4
    source_preference = capital_plan.source_preference
    control_note = readiness.strategic_outlook.replace("_", " ")
    ...
    if (
        pressure.board_reset_risk >= 118
        or pressure.dominant_pressure == "board_reset_risk"
        or state.finance.governance_risk >= 78
    ):
        source_preference = CapitalSourcePreference.BOOTSTRAP
        reserve_share += 4
        go_to_market_share = max(0, go_to_market_share - 2)
    ...
    if queue_exposure.hotspot_lane_overflow > 10:
        reserve_share += 2
        go_to_market_share = max(0, go_to_market_share - 1)
    if queue_exposure.enterprise_queue_risk_accounts > 1:
        reserve_share += 1
    ...
    state.finance.investor_pressure = clamp_int(state.finance.investor_pressure - 2)
    state.finance.covenant_risk = clamp_int(state.finance.covenant_risk - 2)
    state.finance.board_confidence = min(
        100,
        state.finance.board_confidence
        + BALANCE.capital_plan_growth_firebreak_board_confidence_gain
    ...
    return CapitalPlanSummary(
        message=(
            f"Set a terminal solvency council for the {control_note} path. Reserve target "
            f"{format_money(reserve_target)} over {planning_horizon_turns} turns. Allocation now "
            f"P {product_share}% / GTM {go_to_market_share}% / Reserve {reserve_share}% with "
            f"{source_preference.value} capital."
        ),
