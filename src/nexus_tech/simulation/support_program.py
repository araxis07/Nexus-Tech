...
def run_enterprise_lane_mesh(state: GameState) -> SupportOpsActionSummary:
    """Stabilize the top enterprise accounts together when flagship proof is slipping."""

    cost = quantize_money(
        BALANCE.support_program_enterprise_assurance_cost
        + BALANCE.support_program_lane_recovery_cost
    ...
    if not accounts:
        raise ValueError("No enterprise accounts need a lane mesh right now.")
    if state.company.cash_on_hand < cost:
        raise ValueError("Not enough cash to run an enterprise lane mesh this turn.")
    ...
    state.support_program.backlog_queue = max(
        ...
        - (
            ...
            + BALANCE.support_program_lane_recovery_backlog_relief
    ...
    for account in accounts[:3]:
        account.open_tickets = max(
            ...
            account.open_tickets
            - (
                BALANCE.support_program_enterprise_assurance_ticket_relief
                + BALANCE.support_program_lane_recovery_ticket_relief
        ...
        account.sla_breach_risk = clamp_int(
            account.sla_breach_risk
            - (
                BALANCE.support_program_enterprise_assurance_sla_relief
                + BALANCE.support_program_lane_recovery_sla_relief
        ...
        account.ticket_queue_age = max(
            ...
            - (
                ...
                + BALANCE.support_program_lane_recovery_queue_age_relief
        ...
        account.support_load = clamp_int(
            ...
            - (
                BALANCE.support_program_enterprise_assurance_support_load_relief
                + BALANCE.support_program_lane_recovery_support_load_relief
        ...
        account.renewal_health = clamp_int(
            ...
            + (
                ...
                + BALANCE.support_program_lane_recovery_renewal_health_gain
        ...
        account.satisfaction = clamp_int(
            account.satisfaction
            + (
                BALANCE.support_program_enterprise_assurance_satisfaction_gain
                + BALANCE.support_program_lane_recovery_satisfaction_gain
        ...
        account.churn_risk = clamp_int(
            account.churn_risk
            - (
                BALANCE.support_program_enterprise_assurance_churn_relief
                + BALANCE.support_program_lane_recovery_churn_relief
        ...
        _apply_lane_program_relief(
            ...
            + BALANCE.support_program_lane_recovery_lane_relief
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran an enterprise lane mesh across {stabilized_accounts} account(s). "
            f"Cash -{cost}, backlog now {state.support_program.backlog_queue}."
        )
    )


def run_billing_lane_mesh(state: GameState) -> SupportOpsActionSummary:
    """Stabilize the highest-risk billing accounts together under one collections push."""

    cost = quantize_money(
        BALANCE.support_program_billing_stabilization_cost
        + BALANCE.support_program_renewal_sweep_cost
        + BALANCE.support_program_account_rescue_cost
    )
    accounts = [
        account
        for account in sorted(
    ...
    if not accounts:
        raise ValueError("No billing accounts need a lane mesh right now.")
    if state.company.cash_on_hand < cost:
        raise ValueError("Not enough cash to run a billing lane mesh this turn.")
    ...
    state.support_program.backlog_queue = max(
        ...
        - (
            ...
            + BALANCE.support_program_renewal_sweep_backlog_relief
            + BALANCE.support_program_account_rescue_backlog_relief
        ),
    )
    state.support_program.escalation_queue = max(
        0,
    ...
    for account in accounts[:3]:
        account.open_tickets = max(
            ...
            - (
                ...
                + BALANCE.support_program_renewal_sweep_ticket_relief
                + BALANCE.support_program_account_rescue_ticket_relief
            ),
        )
        account.sla_breach_risk = clamp_int(
            account.sla_breach_risk
            - (
                ...
                + BALANCE.support_program_renewal_sweep_sla_relief
                + BALANCE.support_program_account_rescue_sla_relief
            )
        )
        account.ticket_queue_age = max(
            0,
            ...
            - (
                ...
                + BALANCE.support_program_renewal_sweep_queue_age_relief
                + BALANCE.support_program_account_rescue_queue_age_relief
            ),
        )
        account.support_load = clamp_int(
            account.support_load
            - (
                ...
                + BALANCE.support_program_renewal_sweep_support_load_relief
                + BALANCE.support_program_account_rescue_support_load_relief
            )
        )
        account.invoice_risk = clamp_int(
            account.invoice_risk
            - (
                ...
                + BALANCE.support_program_renewal_sweep_billing_invoice_relief
                + BALANCE.support_program_account_rescue_billing_invoice_relief
            )
        )
        account.failed_payment_risk = clamp_int(
            account.failed_payment_risk
        ...
        account.dunning_steps = max(
            ...
            - (
                ...
                + BALANCE.support_program_account_rescue_billing_dunning_relief
            ),
        )
        account.renewal_health = clamp_int(
            account.renewal_health
            + (
                ...
                + BALANCE.support_program_renewal_sweep_renewal_health_gain
                + BALANCE.support_program_route_billing_renewal_health_gain
            )
        )
        account.satisfaction = clamp_int(
            ...
            + (
                ...
                + BALANCE.support_program_renewal_sweep_satisfaction_gain
                + BALANCE.support_program_account_rescue_satisfaction_gain
            )
        )
        account.churn_risk = clamp_int(
            account.churn_risk
            - (
                ...
                + BALANCE.support_program_renewal_sweep_churn_relief
                + BALANCE.support_program_account_rescue_churn_relief
            )
        )
        account.escalation_count = max(0, account.escalation_count - 2)
        _apply_lane_program_relief(
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran a billing lane mesh across {stabilized_accounts} account(s). "
            f"Cash -{cost}, covenant risk now {state.finance.covenant_risk}."
        )
    )


def run_onboarding_lane_mesh(state: GameState) -> SupportOpsActionSummary:
    """Stabilize onboarding drag across the lane before implementation debt compounds."""
    ...
    if not accounts:
        raise ValueError("No onboarding accounts need a lane mesh right now.")
    if state.company.cash_on_hand < cost:
        raise ValueError("Not enough cash to run an onboarding lane mesh this turn.")
    ...
    state.company.reputation = clamp_int(
        state.company.reputation + BALANCE.support_program_onboarding_recovery_board_pressure_relief
    )
    return SupportOpsActionSummary(
        message=(
            f"Ran an onboarding lane mesh across {stabilized_accounts} account(s). "
            f"Cash -{cost}, onboarding queue now {state.support_program.backlog_queue}."
        )
    )


def run_white_glove_lane_mesh(state: GameState) -> SupportOpsActionSummary:
    """Stabilize the highest-touch accounts together before premium drag spreads."""

    cost = quantize_money(
        BALANCE.support_program_white_glove_recovery_cost
        + BALANCE.support_program_white_glove_renewal_guard_cost
    ...
    accounts = [
        ...
        for account in sorted(
            state.customer_accounts,
            key=lambda account: (
                account.support_tier in {SupportTier.PRIORITY, SupportTier.WHITE_GLOVE},
                account.contract_value,
                _calculate_account_support_severity(account),
                100 - account.renewal_health,
            ),
        ...
        and (
            account.support_tier in {SupportTier.PRIORITY, SupportTier.WHITE_GLOVE}
            or account.contract_value >= Decimal("3200.00")
        )
    ]
    if not accounts:
        raise ValueError("No premium accounts need a white-glove lane mesh right now.")
    if state.company.cash_on_hand < cost:
        raise ValueError("Not enough cash to run a white-glove lane mesh this turn.")
    ...
    for account in accounts[:3]:
        account.open_tickets = max(
            ...
            account.open_tickets
            - (
                BALANCE.support_program_white_glove_recovery_ticket_relief
                + BALANCE.support_program_white_glove_renewal_guard_ticket_relief
        ...
        account.sla_breach_risk = clamp_int(
            account.sla_breach_risk
            - (
                BALANCE.support_program_white_glove_recovery_sla_relief
                + BALANCE.support_program_white_glove_renewal_guard_sla_relief
        ...
        account.ticket_queue_age = max(
            ...
            account.ticket_queue_age
            - (
                BALANCE.support_program_white_glove_recovery_queue_age_relief
        ...
        account.support_load = clamp_int(
            account.support_load
            - (
                BALANCE.support_program_white_glove_recovery_support_load_relief
                + BALANCE.support_program_white_glove_renewal_guard_support_load_relief
        ...
        account.invoice_risk = clamp_int(
            account.invoice_risk
            - (
                BALANCE.support_program_white_glove_recovery_invoice_relief
                + BALANCE.support_program_white_glove_renewal_guard_invoice_relief
            )
        )
        account.failed_payment_risk = clamp_int(
            account.failed_payment_risk
            - (
                BALANCE.support_program_white_glove_recovery_payment_relief
        ...
        account.renewal_health = clamp_int(
            account.renewal_health
            + (
                BALANCE.support_program_white_glove_recovery_renewal_health_gain
        ...
        account.satisfaction = clamp_int(
            account.satisfaction
            + (
                BALANCE.support_program_white_glove_recovery_satisfaction_gain
                + BALANCE.support_program_white_glove_renewal_guard_satisfaction_gain
        ...
        account.churn_risk = clamp_int(
            account.churn_risk
            - (
                BALANCE.support_program_white_glove_recovery_churn_relief
                + BALANCE.support_program_white_glove_renewal_guard_churn_relief
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran a white-glove lane mesh across {stabilized_accounts} account(s). "
            f"Cash -{cost}, board confidence now {state.finance.board_confidence}."
        )
    )


def run_enterprise_reference_watch(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run one more flagship enterprise follow-up loop after the lane-mesh phase."""
    ...
    council_total = quantize_money(
        BALANCE.support_program_white_glove_backstop_cost
        + BALANCE.support_program_white_glove_reference_committee_cost * 10
        + BALANCE.support_program_enterprise_reference_cycle_cost * 10
        + BALANCE.support_program_enterprise_renewal_cabinet_cost * 9
    )
    total_cost = quantize_money(council_total + extra_cost)
    if account.status is CustomerAccountStatus.CHURNED:
        raise ValueError("That account has already churned.")
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run an enterprise reference watch this turn.")
    if (
        account.segment is not MarketSegment.ENTERPRISE
        and account.support_tier is SupportTier.STANDARD
        and account.contract_value < Decimal("3000.00")
    ):
        raise ValueError("That account is not exposed enough for an enterprise reference watch.")
    if (
        ...
        raise ValueError("That account does not need an enterprise reference watch right now.")

    run_enterprise_reference_council(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
...
def run_billing_renewal_watch(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run one more billing follow-up loop on one account after lane-level containment."""
    ...
    council_total = quantize_money(
        BALANCE.support_program_billing_stabilization_cost
        + BALANCE.support_program_billing_retention_reset_cost * 21
        + BALANCE.support_program_billing_covenant_reset_cost * 19
    )
    total_cost = quantize_money(council_total + extra_cost)
    if account.status is CustomerAccountStatus.CHURNED:
        raise ValueError("That account has already churned.")
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run a billing renewal watch this turn.")
    if (
        ...
        and account.dunning_steps <= 1
        and account.renewal_health >= 82
        and account.churn_risk <= 14
    ):
        raise ValueError("That account does not need a billing renewal watch right now.")

    run_billing_liquidity_council(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.lane_focus = SupportLaneFocus.BILLING
    state.support_program.backlog_queue = max(
...
def run_onboarding_go_live_watch(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run one more onboarding follow-up loop on one account after lane-level recovery."""
    ...
    council_total = quantize_money(
        BALANCE.support_program_onboarding_recovery_cost * 22
        + BALANCE.support_program_onboarding_fast_track_cost * 20
    )
    total_cost = quantize_money(council_total + extra_cost)
    if account.status is CustomerAccountStatus.CHURNED:
        raise ValueError("That account has already churned.")
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run an onboarding go-live watch this turn.")
    if (
        classify_account_support_lane(account) is not SupportLaneFocus.ONBOARDING
        and account.onboarding_health >= 82
        and account.renewal_health >= 80
        and account.sla_breach_risk <= 14
        and account.open_tickets <= 1
        and account.churn_risk <= 14
    ):
        raise ValueError("That account does not need an onboarding go-live watch right now.")

    run_onboarding_continuity_council(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.lane_focus = SupportLaneFocus.ONBOARDING
    state.support_program.backlog_queue = max(
...
def run_white_glove_retention_watch(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run a concentrated premium-account retention loop after lane-level stabilization."""
    ...
    if account.status is CustomerAccountStatus.CHURNED:
        raise ValueError("That account has already churned.")
    if state.company.cash_on_hand < cost:
        raise ValueError("Not enough cash to run a white-glove retention watch this turn.")
    if (
        ...
        raise ValueError("That account is not exposed enough for a white-glove retention watch.")
    if (
        account.open_tickets <= 1
        ...
        raise ValueError("That account does not need a white-glove retention watch right now.")
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran a white-glove retention watch for {account.name}. Cash -{cost}, "
            f"renewal health now {account.renewal_health}."
        )
    )
...
def run_onboarding_continuity_commission(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run the last onboarding continuity escalation after the authority tier saturates."""

    base_cost = quantize_money(
        BALANCE.support_program_onboarding_recovery_cost * 15
        + BALANCE.support_program_onboarding_fast_track_cost * 13
    )
    extra_cost = quantize_money(
        BALANCE.support_program_onboarding_recovery_cost * 2
        + BALANCE.support_program_onboarding_fast_track_cost * 2
    ...
    total_cost = quantize_money(base_cost + extra_cost)
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run an onboarding continuity commission this turn.")

    run_onboarding_continuity_authority(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.backlog_queue = max(
        0,
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran an onboarding continuity commission for {account.name}. Cash -{total_cost}, "
            f"onboarding health now {account.onboarding_health}."
        )
    )


def run_onboarding_continuity_oversight(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run one more onboarding continuity loop after the commission tier saturates."""

    base_cost = quantize_money(
        BALANCE.support_program_onboarding_recovery_cost * 17
        + BALANCE.support_program_onboarding_fast_track_cost * 15
    )
    extra_cost = quantize_money(
        BALANCE.support_program_onboarding_recovery_cost * 2
        + BALANCE.support_program_onboarding_fast_track_cost * 2
    ...
    total_cost = quantize_money(base_cost + extra_cost)
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run an onboarding continuity oversight this turn.")

    run_onboarding_continuity_commission(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.backlog_queue = max(
        0,
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran an onboarding continuity oversight for {account.name}. Cash -{total_cost}, "
            f"onboarding health now {account.onboarding_health}."
        )
    )


def run_onboarding_continuity_council(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run the council-tier onboarding continuity loop after the oversight tier saturates."""

    base_cost = quantize_money(
        BALANCE.support_program_onboarding_recovery_cost * 19
        + BALANCE.support_program_onboarding_fast_track_cost * 17
    )
    extra_cost = quantize_money(
        BALANCE.support_program_onboarding_recovery_cost * 3
        + BALANCE.support_program_onboarding_fast_track_cost * 3
    ...
    total_cost = quantize_money(base_cost + extra_cost)
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run an onboarding continuity council this turn.")

    run_onboarding_continuity_oversight(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.backlog_queue = max(
        0,
    ...
    state.finance.board_confidence = clamp_int(state.finance.board_confidence + 3)
    state.company.reputation = clamp_int(state.company.reputation + 2)
    return SupportOpsActionSummary(
        message=(
            f"Ran an onboarding continuity council for {account.name}. Cash -{total_cost}, "
            f"onboarding health now {account.onboarding_health}."
        )
    )
...
def run_enterprise_reference_commission(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run the last enterprise proof escalation after the authority tier saturates."""

    base_cost = quantize_money(
        BALANCE.support_program_white_glove_backstop_cost
        + BALANCE.support_program_white_glove_reference_committee_cost * 6
        + BALANCE.support_program_enterprise_reference_cycle_cost * 6
        + BALANCE.support_program_enterprise_renewal_cabinet_cost * 5
    )
    extra_cost = quantize_money(
        BALANCE.support_program_white_glove_reference_committee_cost
        + BALANCE.support_program_enterprise_reference_cycle_cost
    ...
    total_cost = quantize_money(base_cost + extra_cost)
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run an enterprise reference commission this turn.")

    run_enterprise_reference_authority(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.backlog_queue = max(
        0,
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran an enterprise reference commission for {account.name}. Cash -{total_cost}, "
            f"renewal health now {account.renewal_health}."
        )
    )


def run_enterprise_reference_oversight(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run one more enterprise proof loop after the commission tier saturates."""

    base_cost = quantize_money(
        BALANCE.support_program_white_glove_backstop_cost
        + BALANCE.support_program_white_glove_reference_committee_cost * 7
        + BALANCE.support_program_enterprise_reference_cycle_cost * 7
        + BALANCE.support_program_enterprise_renewal_cabinet_cost * 6
    )
    extra_cost = quantize_money(
        BALANCE.support_program_white_glove_reference_committee_cost
        + BALANCE.support_program_enterprise_reference_cycle_cost
    ...
    total_cost = quantize_money(base_cost + extra_cost)
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run an enterprise reference oversight this turn.")

    run_enterprise_reference_commission(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.backlog_queue = max(
        0,
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran an enterprise reference oversight for {account.name}. Cash -{total_cost}, "
            f"renewal health now {account.renewal_health}."
        )
    )


def run_enterprise_reference_council(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run the council-tier enterprise proof loop after the oversight tier saturates."""

    base_cost = quantize_money(
        BALANCE.support_program_white_glove_backstop_cost
        + BALANCE.support_program_white_glove_reference_committee_cost * 8
        + BALANCE.support_program_enterprise_reference_cycle_cost * 8
        + BALANCE.support_program_enterprise_renewal_cabinet_cost * 7
    )
    extra_cost = quantize_money(
        BALANCE.support_program_white_glove_reference_committee_cost * 2
        + BALANCE.support_program_enterprise_reference_cycle_cost * 2
    ...
    total_cost = quantize_money(base_cost + extra_cost)
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run an enterprise reference council this turn.")

    run_enterprise_reference_oversight(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.backlog_queue = max(
        0,
        state.support_program.backlog_queue
        - (
            BALANCE.support_program_white_glove_reference_committee_backlog_relief * 2
            + BALANCE.support_program_enterprise_reference_cycle_backlog_relief * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_backlog_relief * 2
        ),
    )
    state.support_program.escalation_queue = max(
        0,
        state.support_program.escalation_queue
        - (
            BALANCE.support_program_white_glove_reference_committee_escalation_relief * 2
            + BALANCE.support_program_enterprise_reference_cycle_escalation_relief * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_escalation_relief * 2
        ),
    )
    account.open_tickets = max(
        0,
        account.open_tickets
        - (
            BALANCE.support_program_white_glove_reference_committee_ticket_relief * 2
            + BALANCE.support_program_enterprise_reference_cycle_ticket_relief * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_ticket_relief * 2
        ),
    )
    account.sla_breach_risk = clamp_int(
        account.sla_breach_risk
        - (
            BALANCE.support_program_white_glove_reference_committee_sla_relief * 2
            + BALANCE.support_program_enterprise_reference_cycle_sla_relief * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_sla_relief * 2
        )
    )
    account.ticket_queue_age = max(
        0,
        account.ticket_queue_age
        - (
            BALANCE.support_program_white_glove_reference_committee_queue_age_relief * 2
            + BALANCE.support_program_enterprise_reference_cycle_queue_age_relief * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_queue_age_relief * 2
        ),
    )
    account.support_load = clamp_int(
        account.support_load
        - (
            BALANCE.support_program_white_glove_reference_committee_support_load_relief * 2
            + BALANCE.support_program_enterprise_reference_cycle_support_load_relief * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_support_load_relief * 2
        )
    )
    account.onboarding_health = clamp_int(
        account.onboarding_health
        + (
            BALANCE.support_program_enterprise_reference_cycle_onboarding_health_gain * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_onboarding_health_gain * 2
        )
    )
    account.renewal_health = clamp_int(
        account.renewal_health
        + (
            BALANCE.support_program_white_glove_reference_committee_renewal_health_gain * 2
            + BALANCE.support_program_enterprise_reference_cycle_renewal_health_gain * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_renewal_health_gain * 2
        )
    )
    account.satisfaction = clamp_int(
        account.satisfaction
        + (
            BALANCE.support_program_white_glove_reference_committee_satisfaction_gain * 2
            + BALANCE.support_program_enterprise_reference_cycle_satisfaction_gain * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_satisfaction_gain * 2
        )
    )
    account.expansion_potential = clamp_int(
        account.expansion_potential
        + (
            BALANCE.support_program_white_glove_reference_committee_expansion_gain * 2
            + BALANCE.support_program_enterprise_reference_cycle_expansion_gain * 2
        )
    )
    account.churn_risk = clamp_int(
        account.churn_risk
        - (
            BALANCE.support_program_white_glove_reference_committee_churn_relief * 2
            + BALANCE.support_program_enterprise_reference_cycle_churn_relief * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_churn_relief * 2
        )
    )
    account.escalation_count = max(0, account.escalation_count - 1)
    account.support_tier = SupportTier.WHITE_GLOVE
    _apply_lane_program_relief(
        state.support_program,
        SupportLaneFocus.ENTERPRISE,
        BALANCE.support_program_white_glove_reference_committee_lane_relief * 2
        + BALANCE.support_program_enterprise_reference_cycle_lane_relief * 2
        + BALANCE.support_program_enterprise_renewal_cabinet_lane_relief * 2,
    )
    state.finance.board_pressure = clamp_int(
        state.finance.board_pressure
        - (
            BALANCE.support_program_white_glove_reference_committee_board_pressure_relief * 2
            + BALANCE.support_program_enterprise_reference_cycle_board_pressure_relief * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_board_pressure_relief * 2
        )
    )
    state.finance.board_confidence = clamp_int(
        state.finance.board_confidence
        + (
            BALANCE.support_program_white_glove_reference_committee_board_confidence_gain * 2
            + BALANCE.support_program_enterprise_reference_cycle_board_confidence_gain * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_board_confidence_gain * 2
        )
    )
    state.finance.board_score = clamp_int(
        state.finance.board_score
        + (
            BALANCE.support_program_white_glove_reference_committee_board_score_gain * 2
            + BALANCE.support_program_enterprise_reference_cycle_score_gain * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_board_score_gain * 2
        )
    )
    state.finance.investor_pressure = clamp_int(
        state.finance.investor_pressure
        - BALANCE.support_program_white_glove_reference_committee_investor_pressure_relief * 2
    )
    state.company.reputation = clamp_int(
        state.company.reputation
        + (
            BALANCE.support_program_white_glove_reference_committee_reputation_gain * 2
            + BALANCE.support_program_enterprise_reference_cycle_reputation_gain * 2
            + BALANCE.support_program_enterprise_renewal_cabinet_reputation_gain * 2
        )
    )
    return SupportOpsActionSummary(
        message=(
            f"Ran an enterprise reference council for {account.name}. Cash -{total_cost}, "
            f"renewal health now {account.renewal_health}."
        )
    )
...
def run_billing_liquidity_commission(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run the last billing containment escalation after the authority tier saturates."""

    base_cost = quantize_money(
        BALANCE.support_program_billing_stabilization_cost
        + BALANCE.support_program_billing_retention_reset_cost * 14
        + BALANCE.support_program_billing_covenant_reset_cost * 12
    )
    extra_cost = quantize_money(
        BALANCE.support_program_billing_retention_reset_cost * 2
        + BALANCE.support_program_billing_covenant_reset_cost * 2
    ...
    total_cost = quantize_money(base_cost + extra_cost)
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run a billing liquidity commission this turn.")

    run_billing_liquidity_authority(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.backlog_queue = max(
        0,
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran a billing liquidity commission for {account.name}. Cash -{total_cost}, "
            f"invoice risk now {account.invoice_risk}."
        )
    )


def run_billing_liquidity_oversight(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run one more billing containment loop after the commission tier saturates."""

    base_cost = quantize_money(
        BALANCE.support_program_billing_stabilization_cost
        + BALANCE.support_program_billing_retention_reset_cost * 16
        + BALANCE.support_program_billing_covenant_reset_cost * 14
    )
    extra_cost = quantize_money(
        BALANCE.support_program_billing_retention_reset_cost * 2
        + BALANCE.support_program_billing_covenant_reset_cost * 2
    ...
    total_cost = quantize_money(base_cost + extra_cost)
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run a billing liquidity oversight this turn.")

    run_billing_liquidity_commission(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.backlog_queue = max(
        0,
    ...
    return SupportOpsActionSummary(
        message=(
            f"Ran a billing liquidity oversight for {account.name}. Cash -{total_cost}, "
            f"invoice risk now {account.invoice_risk}."
        )
    )


def run_billing_liquidity_council(
    state: GameState,
    account_id,
) -> SupportOpsActionSummary:
    """Run the council-tier billing containment loop after the oversight tier saturates."""

    base_cost = quantize_money(
        BALANCE.support_program_billing_stabilization_cost
        + BALANCE.support_program_billing_retention_reset_cost * 18
        + BALANCE.support_program_billing_covenant_reset_cost * 16
    )
    extra_cost = quantize_money(
        BALANCE.support_program_billing_retention_reset_cost * 3
        + BALANCE.support_program_billing_covenant_reset_cost * 3
    ...
    total_cost = quantize_money(base_cost + extra_cost)
    if state.company.cash_on_hand < total_cost:
        raise ValueError("Not enough cash to run a billing liquidity council this turn.")

    run_billing_liquidity_oversight(state, account_id)
    account = _get_account_by_id(state.customer_accounts, account_id)
    state.company.cash_on_hand = quantize_money(state.company.cash_on_hand - extra_cost)
    state.support_program.backlog_queue = max(
        0,
        ...
        - (
            BALANCE.support_program_billing_retention_reset_backlog_relief * 3
            + BALANCE.support_program_billing_covenant_reset_backlog_relief * 3
        ),
    )
    state.support_program.escalation_queue = max(
        0,
        ...
        - (
            BALANCE.support_program_billing_retention_reset_escalation_relief * 3
            + BALANCE.support_program_billing_covenant_reset_escalation_relief * 3
        ),
    )
    account.open_tickets = max(
        0,
        ...
        - (
            BALANCE.support_program_billing_retention_reset_ticket_relief * 3
            + BALANCE.support_program_billing_covenant_reset_ticket_relief * 3
        ),
    )
    account.sla_breach_risk = clamp_int(
        account.sla_breach_risk
        - (
            BALANCE.support_program_billing_retention_reset_sla_relief * 3
            + BALANCE.support_program_billing_covenant_reset_sla_relief * 3
        )
    )
    account.ticket_queue_age = max(
        0,
        ...
        - (
            BALANCE.support_program_billing_retention_reset_queue_age_relief * 3
            + BALANCE.support_program_billing_covenant_reset_queue_age_relief * 3
        ),
    )
    account.support_load = clamp_int(
        account.support_load
        - (
            BALANCE.support_program_billing_retention_reset_support_load_relief * 3
            + BALANCE.support_program_billing_covenant_reset_support_load_relief * 3
        )
    )
    account.invoice_risk = clamp_int(
        account.invoice_risk
        - (
            BALANCE.support_program_billing_retention_reset_invoice_relief * 3
            + BALANCE.support_program_billing_covenant_reset_invoice_relief * 3
        )
    )
    account.failed_payment_risk = clamp_int(
        account.failed_payment_risk
        - (
            BALANCE.support_program_billing_retention_reset_payment_relief * 3
            + BALANCE.support_program_billing_covenant_reset_payment_relief * 3
        )
    )
    account.dunning_steps = max(
        0,
        ...
        - (
            BALANCE.support_program_billing_retention_reset_dunning_relief * 3
            + BALANCE.support_program_billing_covenant_reset_dunning_relief * 3
        ),
    )
    account.renewal_health = clamp_int(
        account.renewal_health
        + (
            BALANCE.support_program_billing_retention_reset_renewal_health_gain * 3
            + BALANCE.support_program_billing_covenant_reset_renewal_health_gain * 3
        )
    )
    account.satisfaction = clamp_int(
        account.satisfaction
        + (
            BALANCE.support_program_billing_retention_reset_satisfaction_gain * 3
            + BALANCE.support_program_billing_covenant_reset_satisfaction_gain * 3
        )
    )
    account.churn_risk = clamp_int(
        account.churn_risk
        - (
            BALANCE.support_program_billing_retention_reset_churn_relief * 3
            + BALANCE.support_program_billing_covenant_reset_churn_relief * 3
        )
    )
    account.escalation_count = max(0, account.escalation_count - 1)
    _apply_lane_program_relief(
        ...
        SupportLaneFocus.BILLING,
        BALANCE.support_program_billing_retention_reset_lane_relief * 3
        + BALANCE.support_program_billing_covenant_reset_lane_relief * 3,
    )
    state.finance.board_pressure = clamp_int(
        state.finance.board_pressure
        - (
            BALANCE.support_program_billing_retention_reset_board_pressure_relief * 3
            + BALANCE.support_program_billing_covenant_reset_board_pressure_relief * 3
        )
    )
    state.finance.investor_pressure = clamp_int(
        state.finance.investor_pressure
        - (
            BALANCE.support_program_billing_retention_reset_investor_pressure_relief * 3
            + BALANCE.support_program_billing_covenant_reset_investor_pressure_relief * 3
        )
    )
    state.finance.covenant_risk = clamp_int(
        state.finance.covenant_risk
    ...
    state.finance.board_confidence = clamp_int(
        ...
        + (
            BALANCE.support_program_billing_retention_reset_board_confidence_gain * 3
            + BALANCE.support_program_billing_covenant_reset_board_confidence_gain * 3
        )
    )
    return SupportOpsActionSummary(
        message=(
            f"Ran a billing liquidity council for {account.name}. Cash -{total_cost}, "
            f"invoice risk now {account.invoice_risk}."
        )
    )
